// Generated from Les3Parser.ecs by LeMP custom tool. LeMP version: 2.6.4.0
...
namespace Loyc.Syntax.Les
{
	...
	partial class Les3Parser {
		...
		public VList<LNode> ExprList(ref TokenType endMarker, VList<LNode> list = default(VList<LNode>), bool isBracedBlock = false)
		{
			...
			// Line 87: ((&{isBracedBlock} (TT.RBrack|TT.RParen))? (TT.Comma|TT.Newline|TT.Semicolon) NewlinesOpt ({..} / TopExpr) ErrorTokensOpt)*
			for (;;) {
				switch ((TT) LA0) {
			...
			match1:
				{
					// Line 87: (&{isBracedBlock} (TT.RBrack|TT.RParen))?
					la0 = (TT) LA0;
		...
		public IEnumerable<LNode> ExprListLazy(Holder<TokenType> endMarker)
		{
			...
			// Line 113: ((TT.Comma|TT.Newline|TT.Semicolon) NewlinesOpt ({..} / TopExpr) ErrorTokensOpt)*
			for (;;) {
				la0 = (TT) LA0;
		...
		protected LNode TopExpr()
		{
			...
			for (;;) {
				...
				if (la0 == TT.At) {
					...
					// Line 133: greedy(~(EOF|TT.LBrace|TT.Newline) => {..})?
					la0 = (TT) LA0;
		...
		LNode Expr(Precedence context)
		{
			...
			// Line 162: (KeywordExpression | PrefixExpr greedy( &{context.CanParse(P.Primary)} FinishPrimaryExpr | &{CanParse(context, $LI, out prec)} InfixOperatorName Expr | &{context.CanParse(_prec.Find(OperatorShape.Suffix, LT($LI).Value))} TT.PreOrSufOp | &{context.CanParse(P.Of)} TT.Not (TT.LParen ExprList TT.RParen / Expr) )*)
			la0 = (TT) LA0;
			if (la0 == TT.Keyword)
				e = KeywordExpression();
			else {
				...
				// Line 167: greedy( &{context.CanParse(P.Primary)} FinishPrimaryExpr | &{CanParse(context, $LI, out prec)} InfixOperatorName Expr | &{context.CanParse(_prec.Find(OperatorShape.Suffix, LT($LI).Value))} TT.PreOrSufOp | &{context.CanParse(P.Of)} TT.Not (TT.LParen ExprList TT.RParen / Expr) )*
				for (;;) {
					switch ((TT) LA0) {
					...
					case TT.Not:
						{
							if (context.CanParse(P.Of)) {
								...
								int endIndex;
								// Line 182: (TT.LParen ExprList TT.RParen / Expr)
								la0 = (TT) LA0;
								if (la0 == TT.LParen) {
									Skip();
									args = ExprList(args);
		...
		Symbol InfixOperatorName(out Token op)
		{
			...
			// Line 190: ( (TT.Assignment|TT.Dot|TT.NormalOp) (TT.Newline)* | &{(TT) LA($LI + 1) != TT.Newline} TT.Colon | &!{IsContinuator(LT($LI).Value)} TT.Id (&{op.EndIndex == LT0.StartIndex} (TT.Assignment|TT.Dot|TT.NormalOp) / {..}) (TT.Newline (TT.Newline)* / {..}) )
			switch ((TT) LA0) {
			case TT.Assignment: case TT.Dot: case TT.NormalOp:
				{
					op = MatchAny();
			...
			default:
				{
					...
					// Line 205: (TT.Newline (TT.Newline)* / {..})
					la0 = (TT) LA0;
					if (la0 == TT.Newline) {
						Skip();
						// Line 205: (TT.Newline)*
		...
		LNode FinishPrimaryExpr(LNode e)
		{
			TT la0;
			LNode result = default(LNode);
			// Line 216: (CallArgs | TT.LBrack ExprList TT.RBrack)
			la0 = (TT) LA0;
			if (la0 == TT.LParen)
				result = CallArgs(e);
			else {
		...
		LNode Particle(bool inAttribute = false)
		{
			...
			Token lit_rpar = default(Token);
			LNode result = default(LNode);
			// Line 246: ( (TT.BQId|TT.Id) | TT.Literal | TT.SingleQuoteOp TokenList | BracedBlock | SquareBracketList | TT.LParen ExprList TT.RParen )
			switch ((TT) LA0) {
