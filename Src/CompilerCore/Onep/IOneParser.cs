...
namespace Loyc.CompilerCore.Onep
{
		...
		where Token : ITokenValue
	{
		...
		/// <param name="source">A list of tokens.</param>
		...
		/// position should be between the start position and source.Length, but is 
		/// otherwise undefined.
		/// </param>
		/// <param name="untilEnd">If true, the parser will try to parse until the end 
		/// of the string. If false, it will parse until it can parse no more.</param>
	...
	/// </code>
	/// </remarks>
	public interface IOneParser<Expr,Token> : 
		IExprParser<Expr,Token>, IEnumerable<IOneOperator<Expr,Token>>
		where Token : ITokenValue
	{
		void Add(IOneOperator<Expr,Token> op);
		void AddRange(IEnumerable<IOneOperator<Expr,Token>> ops);
		void Clear();
