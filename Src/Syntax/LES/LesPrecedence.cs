...
namespace Loyc.Syntax.Les
{
	...
	/// <li>Power: binary **</li>
	/// <li>DoubleTilde: binary right-associative ~~</li>
	...
	/// they are immiscible with the DoubleTilde, Multiply, Arrow, Add, AndBits, 
	/// OrBits, OrIfNull, and Range operators, as explained in the documentation 
	/// of <see cref="Precedence"/>.
	/// <para/>
	/// I noticed that none of the high-precedence operators were right-
	/// associative, so I added the ~~ operator to "fill in the gap". Similarly,
	/// noticing that there were no prefix operators with low precedence, I added
	/// ".." which has a precedence lower than anything except attributes.
	/// <para/>
	...
	/// <para/>
	/// The ++, --, and $ operators also have two roles, but different roles: 
	...
	/// prefix operator. Clearly <c>x |?| y</c> and <c>x |?| |?| y</c> make sense, 
	/// but <c>x |?| + y</c> is ambiguous: it could be parsed as <c>(x |?|) + y</c> 
	/// or <c>x |?| (+ y)</c>. It turns out that a computer language can contain 
	/// operators that serve as both binary and prefix operators OR it can contain 
	/// operators that serve as binary and suffix operators, but a language is 
	/// ambiguous if it has both kinds of operators at the same time.
	/// <para/>
	...
	/// only operators derived from <c>++</c>, <c>--</c> and <c>$</c> can be 
	...
	/// <li>If the last character is '=', the precedence is Assign.</li>
	...
	/// parser; C-style conditional expressions can still be parsed in LEL with
	/// the help of a macro, but they are generally not necessary since the 
	...
	public static class LesPrecedence
	{
		...
		public static readonly Precedence DoubleTilde= new Precedence(78,  79, 79,78);
		...
		public static readonly Precedence Backtick   = new Precedence(45,  73, 46,72);
		...
		public static readonly Precedence PrefixDots = new Precedence(-10, -9, -9,-10);
	}
}
