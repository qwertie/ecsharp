...
namespace Loyc.BooStyle.Tests
{
	...
	class Benchmark
	{
        ...
        /// machine, Array.Copy can copy 256 bytes 2,000,000 times in 150 ms.
        /// Array.Copy requires 9 times as long to do the same operation 16 bytes at
        /// a time, which suggests that Array.Copy has a very high call overhead and
        /// should not be used to copy small arrays. About half of the 150 ms is
        /// probably overhead, so the theoretical optimum speed must be under 100
        /// ms.
        /// 
        /// The fastest equivalent with purely managed code is to copy 32 bits at a
        /// time with a pair of pinned pointers, and this takes 312 ms.
        /// 
        ...
        /// 
        /// Most strangely of all, the benchmark shows copying from one array to
        /// another is generally faster than reading a single array.
        /// 
        ...
        /// and this only takes around 578 ms (it varies), versus 312 ms if the
        /// array is pinned once per 256 bytes, and 796 ms when copying one byte at
        /// a time with verifiable code.
        /// 
        ...
        /// 
        /// All these strange results prompted me to check out the assembly code. I
        ...
        ///
        /// Most notably, the JIT makes somewhat poor use of x86's limited
        /// registers. It rarely uses ecx in this benchmark's inner loops; it
        /// frequently copies values between eax and edx, effectively wasting one of
        /// them; and it never once uses edi. Consequently, local variables often
        /// have to be saved onto the stack in cases where all relevant values could
        /// easily fit in registers. It does not cache an array's length in a
		/// register, and the loop counter of certain inner loops is kept on the 
		/// stack.
        /// 
        ...
        ///
        ///     while (left-- > 0) total += *p++;
        ///
        /// is slower than the for-loop alternative:
        /// 
        ///     for (int B = 0; B &lt; array1.Length; B++) total2 += p[B];
        ///
		...
		/// unrolled loop, so I added read test #5 and write test #5. Sadly, the
		/// unrolling simply didn't help.
