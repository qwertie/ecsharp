...
namespace Loyc.CompilerCore.Onep
{
	...
	{
		protected static readonly Symbol _PUNC = new Symbol("PUNC");
		...
		/// <param name="">Optional: an IOperatorDivider whose MayDivide method 
		...
		{
			Debug.Assert(source.Length <= startPosition + length);
			_tokens.Clear();
			_state.Clear();
			_source = source;
			...
			for(int i = startPosition; i < startPosition + length; i++) 
			{
				Token t = source[i];
				string text = t.Text;
				if (t.Type == _PUNC && text != null && text.Length > 1) {
			...
			Debug.Assert(IndexInOriginalSource(Length-1) == startPosition+length-1);
		}
		...
		where Token : ITokenValue
	{
		public TokenValueWrapper(Token original)
			: base(original.Type, original.Text)
			{ _original = original; }
