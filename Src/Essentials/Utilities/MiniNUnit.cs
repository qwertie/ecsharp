...
namespace Loyc.MiniTest
{
	...
	public class Assert
	{
		...
		public static new bool Equals(object a, object b) { AreEqual(a, b); }
		...
		{
			Require(a == b, "References are not equal: {0} != {1}.".Localize(a, b));
		}
		...
		class AListTests
		{
			...
			[Test(10000)]
			public AListTests(int iterations) {
			}

			[TestFixture("food")]
			IEnumerable<object> TestCases(string s) { yield return ... }

			[Test]
			IEnumerable<Func<int,int>> TestFunctions() { yield return ... }
		}*/
		...
		static public void Pass(string message, params object[] args)
		{
			...
			throw new SuccessException(message.Localize(args)));
		}
