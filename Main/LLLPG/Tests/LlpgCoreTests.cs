...
{
	/// <summary>Helper methods for making LLLPG grammar nodes. Used by <see cref="LlpgCoreTests"/>.</summary>
	...
	public class LlpgCoreTests : LlpgHelpers
	{
		...
		{
			string output = EcsLanguageService.WithPlainCSharpPrinter.Print(result, MessageSink.Default);
			Assert.AreEqual(StripExtraWhitespace(verbatim), StripExtraWhitespace(output));
		}
		public static string StripExtraWhitespace(string a)
		{
			return LeMP.TestCompiler.StripExtraWhitespace(a);
		}
		...
		public void AddNumbers()
		{
			...
			Number.Basis = (LNode)F.Attr(F.Public, F.Fn(F.Int32, F.Id("Number"), F.AltList()));
...
//				}");
		...
		public void RuleRefWithArgs()
		{
			...
			NTokens.Basis = F.Fn(F.Void, F.Missing, F.AltList(F.Var(F.Int32, "max")));
