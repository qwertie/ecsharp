...
static class ExtraSynchronizers<SyncManager> where SyncManager : ISyncManager
{
	...
	/// <remarks>
	///   <c>Unsafe.As</c> is used because there is no <c>BitConverter</c>-style bitcast
	///   API for an open generic enum type; the alternative (<c>Convert.ToInt64</c>/
	///   <c>Enum.ToObject</c>) boxes twice per value on a hot path.
	/// </remarks>
