...
namespace Ecs.Parser
{
	...
	partial class EcsParser
	{
		...
		LNode IdAtom()
		{
			...
			LNode r;
			#line default
			// Line 170: ( TT.Substitute Atom | TT.operator AnyOperator | (TT.Id|TT.TypeKeyword) | UnusualId )
			switch (LA0) {
			case TT.Substitute:
				{
					var t = MatchAny();
			...
			default:
				{
					var t = UnusualId();
					#line 177 "EcsParserGrammar.les"
					r = IdNode(t);
					#line default
				}
				break;
			}
			#line 178 "EcsParserGrammar.les"
			return r;
			#line default
		}
		bool Scan_IdAtom()
		...
		bool TypeSuffixOpt(bool afterAsOrIs, out Token? dimensionBrack, ref LNode e)
		{
				...
				if (la0 == TT.QuestionMark) {
					...
					if ((count = CountDims(LT(0), true)) > 0) {
						...
						if (la1 == TT.RBrack) {
							...
							#line default
							// Line 255: greedy(&{(count = CountDims(LT($LI), false)) > 0} TT.LBrack TT.RBrack)*
							for (;;) {
								la0 = LA0;
								if (la0 == TT.LBrack) {
			...
			match1:
				{
					var t = MatchAny();
					// Line 243: (&!{afterAsOrIs} | &!(((TT.Add|TT.AndBits|TT.At|TT.Forward|TT.Id|TT.IncDec|TT.LBrace|TT.Literal|TT.LParen|TT.Mul|TT.new|TT.Not|TT.NotBits|TT.Sub|TT.Substitute|TT.TypeKeyword) | UnusualId)))
					if (!afterAsOrIs) {
					} else
		...
		LNode ComplexNameDecl()
		{
			...
			// Line 280: (TT.Dot IdAtom (TParamsDecl)?)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Dot) {
					switch (LA(1)) {
		...
		LNode Atom()
		{
			...
			LNode r;
			#line default
			// Line 380: ( (TT.Dot|TT.Substitute) Atom | TT.operator AnyOperator | (TT.Id|TT.noMacro(base)|TT.noMacro(this)|TT.TypeKeyword) | UnusualId | TT.Literal | ExprInParensAuto | BracedBlock | NewExpr | TokenLiteral | (TT.checked|TT.unchecked) TT.LParen TT.RParen | (TT.noMacro(default)|TT.sizeof|TT.typeof) TT.LParen TT.RParen | TT.delegate TT.LParen TT.RParen TT.LBrace TT.RBrace | TT.is DataType )
			switch (LA0) {
			case TT.Dot:
			case TT.Substitute:
				{
		...
		bool Scan_Atom()
		{
			...
			case TT.@this:
			case TT.TypeKeyword:
				if (!TryMatch((int) TT.Id, (int) TT.@base, (int) TT.@this, (int) TT.TypeKeyword))
					return false;
				break;
			case TT.ContextualKeyword:
				if (!Scan_UnusualId())
		...
		{
			// Line 528: greedy( (TT.ColonColon|TT.Dot|TT.PtrArrow|TT.QuickBind) Atom / PrimaryExpr_NewStyleCast / TT.LParen TT.RParen | TT.LBrack TT.RBrack | TT.IncDec | &(TParams ~(TT.ContextualKeyword|TT.Id)) ((TT.LT|TT.Not) | TT.Dot TT.LBrack) => TParams | BracedBlock )*
			for (;;) {
				switch (LA0) {
				case TT.Dot:
					{
		...
		LNode PrimaryExpr_NewStyleCast(LNode e)
		{
			...
			Symbol kind;
			RWList<LNode> attrs = null;
			// Line 556: ( TT.PtrArrow | TT.as | TT.using )
			la0 = LA0;
			if (la0 == TT.PtrArrow) {
				Skip();
			...
			var type = DataType();
			Match((int) EOF);
			#line 563 "EcsParserGrammar.les"
			if (attrs != null) {
				type = type.PlusAttrs(attrs.ToRVList());
			}
			#line 566 "EcsParserGrammar.les"
			return Up(SetAlternateStyle(SetOperatorStyle(F.Call(kind, e, type, e.Range.StartIndex, rp.EndIndex))));
			#line default
		}
		static readonly HashSet<int> PrefixExpr_set0 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.@checked, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.Dot, (int) TT.DotDot, (int) TT.Forward, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LBrace, (int) TT.Literal, (int) TT.LParen, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.@operator, (int) TT.Power, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked);
		LNode PrefixExpr()
		{
			TokenType la2;
			// Line 574: ( (TT.Add|TT.AndBits|TT.DotDot|TT.Forward|TT.IncDec|TT.Mul|TT.Not|TT.NotBits|TT.Sub) PrefixExpr | (&{Down($LI) && Up(Scan_DataType() && LA0 == EOF)} TT.LParen TT.RParen &!(((TT.Add|TT.BQString|TT.Dot|TT.Sub) | TT.IncDec TT.LParen)) PrefixExpr / TT.Power PrefixExpr / PrimaryExpr) )
			do {
				switch (LA0) {
				case TT.Add:
				case TT.AndBits:
		...
		LNode Expr(Precedence context)
		{
			...
			match3:
				{
					// Line 650: (TT.LT TT.LT Expr | TT.GT TT.GT Expr)
					la0 = LA0;
					if (la0 == TT.LT) {
						Skip();
						Match((int) TT.LT);
		...
		public LNode ExprStart(bool allowUnassignedVarDecl)
		{
			...
			// Line 672: ((TT.Id | UnusualId) TT.Colon ExprStart2 | ExprStart2)
			do {
				la0 = LA0;
		...
		public LNode ExprStart2(bool allowUnassignedVarDecl)
		{
			...
			// Line 686: (&(DetectVarDecl) IdAtom => VarDeclExpr / Expr)
			do {
				switch (LA0) {
				case TT.ContextualKeyword:
				case TT.Id:
			...
			#line 692 "EcsParserGrammar.les"
			if ((attrs != null)) {
				e = e.PlusAttrs(attrs.ToRVList());
			}
			#line 695 "EcsParserGrammar.les"
			return e;
			#line default
		}
		void DetectVarDecl(bool allowUnassigned)
		{
			TokenType la0;
			VarDeclStart();
			// Line 709: ((TT.QuickBindSet|TT.Set) NoUnmatchedColon / &{allowUnassigned} (EOF|TT.Comma))
			la0 = LA0;
		...
		{
			TokenType la0;
			// Line 727: (SubConditional | ~(EOF|TT.Colon|TT.Comma|TT.QuestionMark|TT.Semicolon))*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.QuestionMark)
					SubConditional();
		...
		void SubConditional()
		{
			TokenType la0;
			Skip();
			// Line 732: nongreedy(SubConditional / ~(EOF|TT.Comma|TT.Semicolon))*
			for (;;) {
				la0 = LA0;
		...
		LNode VarDeclExpr()
		{
			...
			#line default
			// Line 737: ((TT.QuickBindSet|TT.Set) Expr)?
			la0 = LA0;
			if (la0 == TT.QuickBindSet || la0 == TT.Set) {
				Skip();
				var init = Expr(ContinueExpr);
				#line 738 "EcsParserGrammar.les"
		...
		static readonly HashSet<int> InParens_ExprOrTuple_set0 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.AttrKeyword, (int) TT.@checked, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.Dot, (int) TT.DotDot, (int) TT.Forward, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LBrace, (int) TT.LBrack, (int) TT.Literal, (int) TT.LParen, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.@operator, (int) TT.Power, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked);
		LNode InParens_ExprOrTuple(bool allowUnassignedVarDecl, int startIndex, int endIndex)
		{
			TokenType la0, la1;
			// Line 778: (ExprStart (TT.Comma (~(EOF))* => (TT.Comma ExprStart | TT.Comma)*)? | )
			la0 = LA0;
			if (InParens_ExprOrTuple_set0.Contains((int) la0)) {
				var e = ExprStart(allowUnassignedVarDecl);
				// Line 779: (TT.Comma (~(EOF))* => (TT.Comma ExprStart | TT.Comma)*)?
				la0 = LA0;
				if (la0 == TT.Comma) {
					#line 780 "EcsParserGrammar.les"
					var list = new RVList<LNode> { 
						e
					};
					#line default
					// Line 781: (TT.Comma ExprStart | TT.Comma)*
					for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							la1 = LA(1);
		...
		{
			TokenType la0, la1;
			// Line 809: (&!{Down($LI) && Up(Try_Scan_AsmOrModLabel(0))} TT.LBrack TT.RBrack)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.LBrack) {
		...
		{
			TokenType la0, la1;
			#line 820 "EcsParserGrammar.les"
			Token attrTarget = default(Token);
			#line default
			// Line 822: greedy((TT.ContextualKeyword|TT.Id|TT.noMacro(return)) TT.Colon)?
			la0 = LA0;
		...
		{
			TokenType la0;
			// Line 846: (TT.AttrKeyword)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.AttrKeyword) {
					var t = MatchAny();
					#line 847 "EcsParserGrammar.les"
		...
		{
			TokenType la0;
			// Line 852: ((TT.AttrKeyword|TT.in))*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.AttrKeyword || la0 == TT.@in) {
					var t = MatchAny();
					#line 853 "EcsParserGrammar.les"
					(attrs = attrs ?? new RWList<LNode>()).Add(IdNode(t));
					#line default
				} else
					break;
		...
		int WordAttributes(ref RWList<LNode> attrs)
		{
			...
			Token t;
			#line default
			// Line 925: (TT.AttrKeyword | ((TT.Id|TT.new|TT.noMacro(this)) | UnusualId) &(( DataType ((TT.AttrKeyword|TT.Id|TT.TypeKeyword) | UnusualId) | (TT.AttrKeyword|TT.new) | TT.noMacro(this) | TT.checked TT.LBrace TT.RBrace | TT.unchecked TT.LBrace TT.RBrace | TT.noMacro(default) TT.Colon | TT.using TT.LParen | (TT.case|TT.class|TT.delegate|TT.do|TT.enum|TT.event|TT.fixed|TT.for|TT.foreach|TT.goto|TT.interface|TT.lock|TT.namespace|TT.noMacro(break)|TT.noMacro(continue)|TT.noMacro(return)|TT.noMacro(throw)|TT.struct|TT.switch|TT.try|TT.while) )))*
			for (;;) {
				switch (LA0) {
				case TT.AttrKeyword:
					{
				...
				case TT.@this:
					{
						if (Try_WordAttributes_Test0(1)) {
							// Line 926: ((TT.Id|TT.new|TT.noMacro(this)) | UnusualId)
							la0 = LA0;
		...
		stop:;
			#line 950 "EcsParserGrammar.les"
			return nonKeywords;
			#line default
		}
		static readonly HashSet<int> Stmt_set0 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.At, (int) TT.Backslash, (int) TT.BQString, (int) TT.@checked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LEGE, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.@operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindSet, (int) TT.Semicolon, (int) TT.Set, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set1 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.At, (int) TT.Backslash, (int) TT.BQString, (int) TT.@checked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LEGE, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.@operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindSet, (int) TT.Set, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set2 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.@as, (int) TT.BQString, (int) TT.@catch, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.@else, (int) TT.EqNeq, (int) TT.@finally, (int) TT.GT, (int) TT.@in, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LT, (int) TT.Mul, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.Semicolon, (int) TT.Set, (int) TT.Sub, (int) TT.@using, (int) TT.@while, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set3 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.@as, (int) TT.BQString, (int) TT.@catch, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.DivMod, (int) TT.DotDot, (int) TT.@else, (int) TT.EqNeq, (int) TT.@finally, (int) TT.GT, (int) TT.@in, (int) TT.IncDec, (int) TT.LambdaArrow, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LT, (int) TT.Mul, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.Semicolon, (int) TT.Set, (int) TT.Sub, (int) TT.@using, (int) TT.@while, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set4 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.At, (int) TT.Backslash, (int) TT.BQString, (int) TT.@checked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.Comma, (int) TT.CompoundSet, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.@operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindSet, (int) TT.RBrace, (int) TT.RBrack, (int) TT.RParen, (int) TT.Semicolon, (int) TT.Set, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set5 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.At, (int) TT.Backslash, (int) TT.BQString, (int) TT.@checked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.@operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindSet, (int) TT.RBrace, (int) TT.RBrack, (int) TT.RParen, (int) TT.Set, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set6 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.@as, (int) TT.BQString, (int) TT.@catch, (int) TT.CompoundSet, (int) TT.DivMod, (int) TT.DotDot, (int) TT.@else, (int) TT.EqNeq, (int) TT.@finally, (int) TT.GT, (int) TT.@in, (int) TT.IncDec, (int) TT.LambdaArrow, (int) TT.LEGE, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuickBind, (int) TT.Semicolon, (int) TT.Set, (int) TT.Sub, (int) TT.@using, (int) TT.@while, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set7 = NewSet((int) TT.At, (int) TT.@checked, (int) TT.ColonColon, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.Dot, (int) TT.Id, (int) TT.@is, (int) TT.LBrace, (int) TT.LBrack, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.@operator, (int) TT.QuestionMark, (int) TT.@sizeof, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked);
		static readonly HashSet<int> Stmt_set8 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.@as, (int) TT.BQString, (int) TT.@catch, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.@else, (int) TT.EqNeq, (int) TT.@finally, (int) TT.GT, (int) TT.@in, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LT, (int) TT.Mul, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.Semicolon, (int) TT.Set, (int) TT.Sub, (int) TT.@using, (int) TT.@while, (int) TT.XorBits);
		static readonly HashSet<int> Stmt_set9 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.AttrKeyword, (int) TT.@checked, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.Dot, (int) TT.DotDot, (int) TT.Forward, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LBrace, (int) TT.LBrack, (int) TT.Literal, (int) TT.LParen, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.@operator, (int) TT.Power, (int) TT.Semicolon, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked);
		public LNode Stmt()
		{
			TokenType la1, la2;
			_stmtAttrs.Clear();
				...
				case TT.@foreach:
					{
						...
						#line 1038 "EcsParserGrammar.les"
						r = r.PlusAttrs(attrs);
						#line default
					}
					break;
		...
		LNode ExprStatement()
		{
			...
			// Line 1056: ((EOF|TT.catch|TT.else|TT.finally|TT.while) =>  | default TT.Semicolon)
			switch (LA0) {
			case EOF:
			case TT.@catch:
			case TT.@else:
		...
		{
			TokenType la0;
			// Line 1065: greedy(~(EOF|TT.LBrace|TT.Semicolon))*
			for (;;) {
				la0 = LA0;
			...
			// Line 1066: greedy(TT.Semicolon | TT.LBrace (TT.RBrace)?)?
			la0 = LA0;
			if (la0 == TT.Semicolon)
				Skip();
			else if (la0 == TT.LBrace) {
		...
		LNode TraitDecl(int startIndex, RVList<LNode> attrs)
		{
			...
			#line 1083 "EcsParserGrammar.les"
			return r;
			#line default
		}
		LNode RestOfSpaceDecl(int startIndex, Symbol kind, RVList<LNode> attrs)
		{
			...
			// Line 1090: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
		...
		LNode AliasDecl(int startIndex, RVList<LNode> attrs)
		{
			...
			#line 1102 "EcsParserGrammar.les"
		...
		LNode UsingDirective(int startIndex, RVList<LNode> attrs)
		{
			...
			// Line 1107: (RestOfAlias | TT.Semicolon)
			la0 = LA0;
			if (la0 == TT.QuickBindSet || la0 == TT.Set) {
		...
		LNode RestOfAlias(int startIndex, LNode newName)
		{
			...
			#line default
			// Line 1119: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
		...
		LNode EnumDecl(int startIndex, RVList<LNode> attrs)
		{
			...
			#line default
			var bases = BaseListOpt();
			// Line 1130: (TT.Semicolon | TT.LBrace TT.RBrace)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				...
				var lb = Match((int) TT.LBrace);
				var rb = Match((int) TT.RBrace);
				#line 1134 "EcsParserGrammar.les"
		...
		{
			TokenType la0;
			// Line 1142: (TT.Colon DataType (TT.Comma DataType)* | )
			la0 = LA0;
			if (la0 == TT.Colon) {
				#line 1142 "EcsParserGrammar.les"
				var bases = new RVList<LNode>();
				#line default
				Skip();
				bases.Add(DataType());
				// Line 1144: (TT.Comma DataType)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
				...
				#line 1146 "EcsParserGrammar.les"
				return F.List();
				#line default
			}
		}
		...
		KeyValuePair<Symbol,LNode> WhereClause()
		{
			...
			#line 1203 "EcsParserGrammar.les"
		...
		{
			TokenType la0;
			// Line 1207: ( (TT.class|TT.struct) | TT.new &{LT($LI).Count == 0} TT.LParen TT.RParen | DataType )
			la0 = LA0;
			if (la0 == TT.@class || la0 == TT.@struct) {
				...
				var lp = Match((int) TT.LParen);
				var rp = Match((int) TT.RParen);
				#line 1209 "EcsParserGrammar.les"
				return F.Call(S.New, n.StartIndex, rp.EndIndex);
				#line default
			} else {
				var t = DataType();
				#line 1210 "EcsParserGrammar.les"
				return t;
				#line default
			}
		}
		...
		LNode AssemblyOrModuleAttribute(int startIndex, RVList<LNode> attrs)
		{
			...
			#line 1233 "EcsParserGrammar.les"
			var list = new RWList<LNode>();
			#line default
			ExprList(list);
		...
		{
			TokenType la0;
			#line 1248 "EcsParserGrammar.les"
			LNode r;
			#line default
			var type = DataType();
			// Line 1252: (&(IdAtom (TT.Comma|TT.Semicolon|TT.Set)) NameAndMaybeInit (TT.Comma NameAndMaybeInit)* TT.Semicolon / ComplexNameDecl (MethodArgListAndBody | WhereClausesOpt MethodBodyOrForward))
			if (Try_MethodOrPropertyOrVar_Test0(0)) {
				...
				// Line 1256: (TT.Comma NameAndMaybeInit)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
				...
				#line 1257 "EcsParserGrammar.les"
				r = F.Call(S.Var, parts, type.Range.StartIndex, end.EndIndex).PlusAttrs(attrs);
				#line default
			} else {
				var name = ComplexNameDecl();
				// Line 1260: (MethodArgListAndBody | WhereClausesOpt MethodBodyOrForward)
				switch (LA0) {
				case TT.LParen:
		...
		LNode OperatorCast(int startIndex, RVList<LNode> attrs)
		{
			...
			#line default
			r = MethodArgListAndBody(startIndex, attrs, S.Fn, type, name);
			#line 1278 "EcsParserGrammar.les"
			return r;
			#line default
		}
		LNode MethodArgListAndBody(int startIndex, RVList<LNode> attrs, Symbol kind, LNode type, LNode name)
		{
			...
			var rp = Match((int) TT.RParen);
			WhereClausesOpt(ref name);
			#line 1284 "EcsParserGrammar.les"
			LNode r, baseCall = null;
			#line default
			// Line 1285: (TT.Colon (TT.noMacro(base)|TT.noMacro(this)) TT.LParen TT.RParen)?
			la0 = LA0;
			if (la0 == TT.Colon) {
				Skip();
				...
				#line 1287 "EcsParserGrammar.les"
			...
			#line default
			// Line 1303: (default TT.Semicolon | MethodBodyOrForward)
			do {
				switch (LA0) {
				case TT.Semicolon:
					goto match1;
		...
		{
			TokenType la0;
			// Line 1327: ( TT.Forward ExprStart TT.Semicolon | TT.LambdaArrow ExprStart TT.Semicolon | TokenLiteral TT.Semicolon | BracedBlock )
			la0 = LA0;
			if (la0 == TT.Forward) {
				var op = MatchAny();
				...
				#line 1330 "EcsParserGrammar.les"
				return body;
				#line default
			}
		}
		LNode NameAndMaybeInit(bool isArray)
		{
			TokenType la0;
			var r = IdAtom();
			// Line 1351: ((TT.QuickBindSet|TT.Set) (&{isArray} &{Down($LI) && Up(HasNoSemicolons())} TT.LBrace TT.RBrace / ExprStart))?
			la0 = LA0;
			if (la0 == TT.QuickBindSet || la0 == TT.Set) {
				Skip();
				// Line 1356: (&{isArray} &{Down($LI) && Up(HasNoSemicolons())} TT.LBrace TT.RBrace / ExprStart)
				do {
					la0 = LA0;
					if (la0 == TT.LBrace) {
				...
				matchExprStart:
					{
						...
						#line 1366 "EcsParserGrammar.les"
		...
		LNode Constructor(int startIndex, RVList<LNode> attrs)
		{
			...
			Token n;
			#line default
			// Line 1380: ( &{_spaceName == LT($LI).Value} (TT.ContextualKeyword|TT.Id) &(TT.LParen TT.RParen (TT.LBrace|TT.Semicolon)) / &{_spaceName != S.Fn || LA($LI + 3) == TT.LBrace} TT.noMacro(this) &(TT.LParen TT.RParen (TT.LBrace|TT.Semicolon)) / (TT.ContextualKeyword|TT.Id|TT.noMacro(this)) &(TT.LParen TT.RParen TT.Colon) )
			do {
				la0 = LA0;
				if (la0 == TT.ContextualKeyword || la0 == TT.Id) {
					if (_spaceName == LT(0).Value) {
			...
			#line 1391 "EcsParserGrammar.les"
			return r;
			#line default
		}
		LNode Destructor(int startIndex, RVList<LNode> attrs)
		{
			...
			#line default
			var tilde = MatchAny();
			// Line 1397: ((&{LT($LI).Value == _spaceName}) (TT.ContextualKeyword|TT.Id) | TT.noMacro(this))
			la0 = LA0;
			if (la0 == TT.ContextualKeyword || la0 == TT.Id) {
				// Line 1397: (&{LT($LI).Value == _spaceName})
				la0 = LA0;
				if (la0 == TT.ContextualKeyword || la0 == TT.Id)
					Check(LT(0).Value == _spaceName, "LT($LI).Value == _spaceName");
				else {
					#line 1398 "EcsParserGrammar.les"
			...
			#line 1404 "EcsParserGrammar.les"
			return r;
			#line default
		}
		LNode DelegateDecl(int startIndex, RVList<LNode> attrs)
		{
			...
			#line 1414 "EcsParserGrammar.les"
			return r.WithAttrs(attrs);
			#line default
		}
		LNode EventDecl(int startIndex, RVList<LNode> attrs)
		{
			...
			var type = DataType();
			var name = ComplexNameDecl();
			// Line 1420: (default (TT.Comma ComplexNameDecl)* TT.Semicolon | BracedBlock)
			do {
				la0 = LA0;
			...
			match1:
				{
					...
					#line default
					// Line 1422: (TT.Comma ComplexNameDecl)*
					for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							Skip();
		...
		{
			var id = Match((int) TT.ContextualKeyword, (int) TT.Id, (int) TT.@default);
			var end = Match((int) TT.Colon);
			#line 1439 "EcsParserGrammar.les"
		...
		LNode CaseStmt(int startIndex)
		{
			...
			var end = Match((int) TT.Colon);
			#line 1446 "EcsParserGrammar.les"
		...
		LNode BlockCallStmt()
		{
			...
			LNode block;
			// Line 1464: ( TT.LParen TT.RParen (BracedBlock | TT.Id => Stmt) | TT.Forward ExprStart TT.Semicolon | BracedBlock )
			la0 = LA0;
			if (la0 == TT.LParen) {
				var lp = MatchAny();
				var rp = Match((int) TT.RParen);
				#line 1464 "EcsParserGrammar.les"
				AppendExprsInside(lp, args, false, true);
				#line default
				// Line 1465: (BracedBlock | TT.Id => Stmt)
				la0 = LA0;
				if (la0 == TT.LBrace)
					block = BracedBlock();
				else {
					block = Stmt();
					#line 1468 "EcsParserGrammar.les"
		...
		LNode ReturnBreakContinueThrow(int startIndex)
		{
			...
			var end = Match((int) TT.Semicolon);
			#line 1498 "EcsParserGrammar.les"
			if (e != null)
		...
		LNode GotoStmt(int startIndex)
		{
			...
			var e = ExprOrNull(false);
			var end = Match((int) TT.Semicolon);
			#line 1508 "EcsParserGrammar.les"
			if (e != null)
		...
		LNode GotoCaseStmt(int startIndex)
		{
			...
			Skip();
			Skip();
			// Line 1516: (TT.noMacro(default) | ExprOpt)
			la0 = LA0;
			if (la0 == TT.@default) {
				la1 = LA(1);
				if (la1 == TT.Semicolon) {
					var @def = MatchAny();
					#line 1517 "EcsParserGrammar.les"
		...
		LNode WhileStmt(int startIndex)
		{
			...
			Match((int) TT.RParen);
			var block = Stmt();
			#line 1545 "EcsParserGrammar.les"
		...
		static readonly HashSet<int> VarIn_set0 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.At, (int) TT.Backslash, (int) TT.BQString, (int) TT.@checked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.CompoundSet, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.@in, (int) TT.IncDec, (int) TT.@is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LEGE, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.@operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindSet, (int) TT.Set, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked, (int) TT.XorBits);
		LNode VarIn()
		{
			TokenType la1;
			#line 1585 "EcsParserGrammar.les"
			LNode @var = null;
			#line default
			// Line 1586: (&(Atom TT.in) Atom / VarDeclStart)
			do {
				switch (LA0) {
				case TT.ContextualKeyword:
				case TT.Id:
			...
			matchVarDeclStart:
				{
					var pair = VarDeclStart();
					#line 1589 "EcsParserGrammar.les"
		...
		static readonly HashSet<int> IfStmt_set0 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.AttrKeyword, (int) TT.@case, (int) TT.@checked, (int) TT.@class, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.@do, (int) TT.Dot, (int) TT.DotDot, (int) TT.@enum, (int) TT.@event, (int) TT.@fixed, (int) TT.@for, (int) TT.@foreach, (int) TT.Forward, (int) TT.@goto, (int) TT.Id, (int) TT.@if, (int) TT.IncDec, (int) TT.@interface, (int) TT.@is, (int) TT.LBrace, (int) TT.LBrack, (int) TT.Literal, (int) TT.@lock, (int) TT.LParen, (int) TT.Mul, (int) TT.@namespace, (int) TT.@new, (int) TT.@base, (int) TT.@break, (int) TT.@continue, (int) TT.@default, (int) TT.@return, (int) TT.@this, (int) TT.@throw, (int) TT.Not, (int) TT.NotBits, (int) TT.@operator, (int) TT.Power, (int) TT.Semicolon, (int) TT.@sizeof, (int) TT.@struct, (int) TT.Sub, (int) TT.Substitute, (int) TT.@switch, (int) TT.@try, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked, (int) TT.@using, (int) TT.@while);
		LNode IfStmt(int startIndex)
		{
			TokenType la0, la1;
			#line 1598 "EcsParserGrammar.les"
			LNode @else = null;
			#line default
			Skip();
			var p = Match((int) TT.LParen);
			...
			#line 1602 "EcsParserGrammar.les"
		...
		LNode SwitchStmt(int startIndex)
		{
			...
			Match((int) TT.RParen);
			var block = Stmt();
			#line 1611 "EcsParserGrammar.les"
		...
		LNode TryStmt(int startIndex)
		{
			...
			LNode expr;
			#line 1647 "EcsParserGrammar.les"
			LNode handler;
			#line default
			// Line 1649: greedy(TT.catch (TT.LParen TT.RParen Stmt / Stmt))*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.@catch) {
					la1 = LA(1);
					if (IfStmt_set0.Contains((int) la1)) {
						var kw = MatchAny();
						// Line 1650: (TT.LParen TT.RParen Stmt / Stmt)
						la0 = LA0;
						if (la0 == TT.LParen) {
							var p = MatchAny();
							Match((int) TT.RParen);
						...
						#line 1653 "EcsParserGrammar.les"
			...
			// Line 1656: greedy(TT.finally Stmt)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.@finally) {
					la1 = LA(1);
					if (IfStmt_set0.Contains((int) la1)) {
						var kw = MatchAny();
						handler = Stmt();
						#line 1657 "EcsParserGrammar.les"
		...
		LNode ExprOrNull(bool allowUnassignedVarDecl = false)
		{
			...
			if (InParens_ExprOrTuple_set0.Contains((int) la0)) {
				...
				#line 1674 "EcsParserGrammar.les"
				return null;
				#line default
			}
		}
		LNode ExprOpt(bool allowUnassignedVarDecl = false)
		{
			...
			if (InParens_ExprOrTuple_set0.Contains((int) la0)) {
				...
				#line 1678 "EcsParserGrammar.les"
				return MissingHere();
				#line default
			}
		}
		static readonly HashSet<int> ExprList_set0 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.AttrKeyword, (int) TT.@checked, (int) TT.Comma, (int) TT.ContextualKeyword, (int) TT.@delegate, (int) TT.Dot, (int) TT.DotDot, (int) TT.Forward, (int) TT.Id, (int) TT.IncDec, (int) TT.@is, (int) TT.LBrace, (int) TT.LBrack, (int) TT.Literal, (int) TT.LParen, (int) TT.Mul, (int) TT.@new, (int) TT.@base, (int) TT.@default, (int) TT.@this, (int) TT.Not, (int) TT.NotBits, (int) TT.@operator, (int) TT.Power, (int) TT.@sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.TypeKeyword, (int) TT.@typeof, (int) TT.@unchecked);
		void ExprList(RWList<LNode> list, bool allowTrailingComma = false, bool allowUnassignedVarDecl = false)
		{
			TokenType la0, la1;
			// Line 1687: nongreedy(ExprOpt (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(ExprOpt(allowUnassignedVarDecl));
				// Line 1688: (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			TokenType la0;
			// Line 1696: nongreedy(ExprOpt (TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(ExprOpt(true));
				// Line 1697: (TT.Comma ExprOpt)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
						...
						#line 1698 "EcsParserGrammar.les"
						Error("Syntax error in argument list");
						#line default
						// Line 1698: (~(EOF|TT.Comma))*
						for (;;) {
							la0 = LA0;
							if (!(la0 == (TokenType) EOF || la0 == TT.Comma))
								Skip();
		...
		{
			TokenType la0;
			#line 1704 "EcsParserGrammar.les"
			LNode e;
			#line default
			// Line 1705: (TT.LBrace TT.RBrace / ExprOpt)
			la0 = LA0;
			if (la0 == TT.LBrace) {
				var lb = MatchAny();
				var rb = Match((int) TT.RBrace);
		...
		{
			TokenType la0, la1;
			// Line 1716: nongreedy(InitializerExpr (TT.Comma EOF / TT.Comma InitializerExpr)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(InitializerExpr());
				// Line 1717: (TT.Comma EOF / TT.Comma InitializerExpr)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			// Line 243: ((TT.Add|TT.AndBits|TT.At|TT.Forward|TT.Id|TT.IncDec|TT.LBrace|TT.Literal|TT.LParen|TT.Mul|TT.new|TT.Not|TT.NotBits|TT.Sub|TT.Substitute|TT.TypeKeyword) | UnusualId)
			switch (LA0) {
			case TT.Add:
			case TT.AndBits:
			case TT.At:
		...
		{
			// Line 579: ((TT.Add|TT.BQString|TT.Dot|TT.Sub) | TT.IncDec TT.LParen)
			switch (LA0) {
			case TT.Add:
			case TT.BQString:
			case TT.Dot:
		...
		static readonly HashSet<int> WordAttributes_Test0_set0 = NewSet((int) TT.@case, (int) TT.@class, (int) TT.@delegate, (int) TT.@do, (int) TT.@enum, (int) TT.@event, (int) TT.@fixed, (int) TT.@for, (int) TT.@foreach, (int) TT.@goto, (int) TT.@interface, (int) TT.@lock, (int) TT.@namespace, (int) TT.@break, (int) TT.@continue, (int) TT.@return, (int) TT.@throw, (int) TT.@struct, (int) TT.@switch, (int) TT.@try, (int) TT.@while);
		...
		{
			TokenType la0;
			// Line 929: ( DataType ((TT.AttrKeyword|TT.Id|TT.TypeKeyword) | UnusualId) | (TT.AttrKeyword|TT.new) | TT.noMacro(this) | TT.checked TT.LBrace TT.RBrace | TT.unchecked TT.LBrace TT.RBrace | TT.noMacro(default) TT.Colon | TT.using TT.LParen | (TT.case|TT.class|TT.delegate|TT.do|TT.enum|TT.event|TT.fixed|TT.for|TT.foreach|TT.goto|TT.interface|TT.lock|TT.namespace|TT.noMacro(break)|TT.noMacro(continue)|TT.noMacro(return)|TT.noMacro(throw)|TT.struct|TT.switch|TT.try|TT.while) )
			switch (LA0) {
			case TT.ContextualKeyword:
			case TT.Id:
			case TT.@operator:
			...
			case TT.TypeKeyword:
				{
					if (!Scan_DataType())
						return false;
					// Line 929: ((TT.AttrKeyword|TT.Id|TT.TypeKeyword) | UnusualId)
					la0 = LA0;
		...
		{
			TokenType la0;
			// Line 1461: ( TT.LParen TT.RParen (TT.LBrace TT.RBrace | TT.Id) | TT.LBrace TT.RBrace | TT.Forward )
			la0 = LA0;
			if (la0 == TT.LParen) {
				if (!TryMatch((int) TT.LParen))
					return false;
				if (!TryMatch((int) TT.RParen))
					return false;
				// Line 1461: (TT.LBrace TT.RBrace | TT.Id)
				la0 = LA0;
				if (la0 == TT.LBrace) {
					if (!TryMatch((int) TT.LBrace))
						return false;
