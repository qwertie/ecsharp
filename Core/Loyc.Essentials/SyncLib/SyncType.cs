...
namespace Loyc.SyncLib
{
	...
	public enum SyncType {
		...
		Unknown = -1,
		/// <summary>This is returned by <see cref="ISyncManager.GetFieldType"/> 
		///   when the reader has determined that the field does not exist, or that it 
		///   does exist but is incompatible with the <c>expectedType</c> parameter.</summary>
		Missing = 0,
		...
		Boolean = 2,
		/// <summary>An 8-bit unsigned integer. This exists as a separate value from 
		///   Unsigned or Integer mainly so that ByteList can be defined as Byte | List, 
		///   because byte lists are treated specially in some software (e.g. byte lists 
		///   often represent file contents, but Unsigned lists do not.)</summary>
		Byte = 3,
		/// <summary>Unsigned integer (data sources often don't support this).</summary>
		Unsigned = 4,
		/// <summary>Signed or unsigned integer of any size (including possibly a byte).</summary>
		Integer = 5,
		/// <summary>Floating-point number of any size.</summary>
		Float = 6,
		/// <summary>Character or code unit (size is implementation-defined). In some 
		///   implementations, Char only appears as part of a CharList, not by itself.</summary>
		Char = 7,
		...
		/// <summary>A synonym for CharList. If possible, ISyncManager implementations should 
		///   treat strings as lists of characters.</summary>
		String = CharList,
	}
}
