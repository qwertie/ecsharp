...
namespace Loyc.Syntax.Les
{
	...
	partial class Les3Parser
	{
		...
		protected LNode CompactExpression(ref TokenType separatorType)
		{
			...
			// Line 80: ( TopExpr greedy(TT.Comma)? | TT.Comma | TT.Semicolon | TT.Newline )
			switch ((TT) LA0) {
		...
		protected LNode NextExpression(ref TokenType separatorType, out Token trailingSeparator, bool isBracedBlock)
		{
			...
			// Line 95: greedy((TT.Comma|TT.Newline|TT.Semicolon) / {..})?
			la0 = (TT) LA0;
			...
			// Line 100: ((EOF|TT.RBrace|TT.RBrack|TT.RParen) => {..})?
			switch ((TT) LA0) {
		...
		public LNodeList ExprList(string listContextName, ref TokenType separatorType, LNode firstItem = null, bool presumeCommaSeparated = true, bool isBracedBlock = false)
		{
			...
			if (firstItem != null)
				list.Add(firstItem);
			NewlinesOpt();
			// Line 139: (&!{isBracedBlock} &!{IsConjoinedToken($LI + 1)} TT.Dot greedy(CompactExpression)* / (TT.SingleQuote TokenListEx / NextExpression)*)
			do {
				la0 = (TT) LA0;
				if (la0 == TT.Dot) {
					if (!isBracedBlock) {
						if (!IsConjoinedToken(0 + 1)) {
							Skip();
							// Line 142: greedy(CompactExpression)*
							for (;;) {
								switch ((TT) LA0) {
								case TT.Assignment: case TT.At: case TT.BackRef: case TT.BQId:
			...
			match2:
				{
					...
					// Line 144: (TT.SingleQuote TokenListEx / NextExpression)*
					for (;;) {
						switch ((TT) LA0) {
						case TT.SingleQuote:
							{
		...
		public IEnumerable<LNode> ExprListLazy(Holder<TokenType> separatorType)
		{
			...
			NewlinesOpt();
			// Line 173: greedy(&!{IsConjoinedToken($LI + 1)} TT.Dot)?
			la0 = (TT) LA0;
			if (la0 == TT.Dot) {
				if (!IsConjoinedToken(0 + 1)) {
					Skip();
			...
			// line 177
			Token trailingSeparator = default;
			// Line 178: (TT.SingleQuote TokenListEx / NextExpression)*
			for (;;) {
				switch ((TT) LA0) {
				case TT.SingleQuote:
					{
			...
			default:
				{
					...
					// Line 203: (TopExpr | (EOF|TT.Comma|TT.Newline|TT.RBrace|TT.RBrack|TT.RParen|TT.Semicolon) => {..})
					switch ((TT) LA0) {
					case TT.Assignment: case TT.At: case TT.BackRef: case TT.BQId:
					case TT.Colon: case TT.Dot: case TT.Id: case TT.LBrace:
					case TT.LBrack: case TT.Literal: case TT.LParen: case TT.NormalOp:
		...
		LNode Expr(Precedence context, bool compactMode)
		{
			...
			// Line 236: (TT.At &!{compactMode && context != Precedence.MinValue} (&{IsConjoinedToken($LI)} TT.At / &{IsConjoinedToken($LI)} => Expr)?  => NewlinesOpt)*
			for (;;) {
				la0 = (TT) LA0;
				if (la0 == TT.At) {
					Skip();
					Check(!(compactMode && context != Precedence.MinValue), "An attribute cannot appear mid-expression in a compact expression list.");
					// Line 242: (&{IsConjoinedToken($LI)} TT.At / &{IsConjoinedToken($LI)} => Expr)?
					la0 = (TT) LA0;
					if (la0 == TT.At) {
			...
			// Line 249: (&{(TT) LA(1) == TT.Id && !compactMode} KeywordExpression / (TT.Dot | PrefixExpr) => PrefixExpr greedy( &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.Primary)} FinishPrimaryExpr | &{!compactMode || IsConjoinedToken($LI)} &{CanParse(context, $LI, out prec)} InfixOperatorName Expr | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(_precMap.Find(OperatorShape.Suffix, LT($LI).Value))} TT.PreOrSufOp | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.SuffixWord)} TT.BQId | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.Of)} TT.Not (TT.LParen ExprList TT.RParen / Expr) | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.Primary)} TT.Keyword )*)
			do {
				la0 = (TT) LA0;
				if (la0 == TT.Dot) {
			...
			match2_a:
				{
					...
					// Line 260: greedy( &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.Primary)} FinishPrimaryExpr | &{!compactMode || IsConjoinedToken($LI)} &{CanParse(context, $LI, out prec)} InfixOperatorName Expr | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(_precMap.Find(OperatorShape.Suffix, LT($LI).Value))} TT.PreOrSufOp | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.SuffixWord)} TT.BQId | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.Of)} TT.Not (TT.LParen ExprList TT.RParen / Expr) | &{!compactMode || IsConjoinedToken($LI)} &{context.CanParse(P.Primary)} TT.Keyword )*
					for (;;) {
						switch ((TT) LA0) {
						...
						case TT.Not:
							{
								if (context.CanParse(P.Of)) {
									if (!compactMode || IsConjoinedToken(0)) {
										...
										// line 284
										int endIndex;
										// Line 285: (TT.LParen ExprList TT.RParen / Expr)
										la0 = (TT) LA0;
										if (la0 == TT.LParen) {
											Skip();
		...
		Symbol InfixOperatorName(out Token op, bool compactMode)
		{
			...
			// Line 302: ( (TT.Assignment|TT.Dot|TT.NormalOp) (TT.Newline)* | &{(TT) LA($LI + 1) != TT.Newline} TT.Colon | &{!IsContinuator(LT($LI).Value) && !compactMode} TT.Id (&{op.EndIndex == LT0.StartIndex} (TT.Assignment|TT.Dot|TT.NormalOp) / {..}) (TT.Newline (TT.Newline)* / {..}) )
			switch ((TT) LA0) {
			case TT.Assignment: case TT.Dot: case TT.NormalOp:
				{
					op = MatchAny();
			...
			default:
				{
					...
					op = Match((int) TT.Id);
					// Line 307: (&{op.EndIndex == LT0.StartIndex} (TT.Assignment|TT.Dot|TT.NormalOp) / {..})
					do {
						la0 = (TT) LA0;
						if (la0 == TT.Dot) {
					...
					// Line 317: (TT.Newline (TT.Newline)* / {..})
					la0 = (TT) LA0;
					if (la0 == TT.Newline) {
						Skip();
						// Line 317: (TT.Newline)*
		...
		LNode FinishPrimaryExpr(LNode e)
		{
			...
			LNode result = default(LNode);
			// Line 328: (CallArgs | TT.LBrack ExprList TT.RBrack)
			la0 = (TT) LA0;
			if (la0 == TT.LParen)
				result = CallArgs(e);
			else {
		...
		LNode PrefixExpr(Precedence context, bool compactMode)
		{
			...
			LNode result = default(LNode);
			// Line 344: (&{!compactMode || LT(0).EndIndex == LT(1).StartIndex} (TT.Assignment|TT.Colon|TT.NormalOp|TT.Not|TT.PrefixOp|TT.PreOrSufOp) NewlinesOpt Expr / Particle)
			switch ((TT) LA0) {
		...
		LNode Particle(bool compactMode = false)
		{
			...
			Token lit_rpar = default(Token);
			LNode result = default(LNode);
			// Line 360: ( (TT.BQId|TT.Id) | TT.Literal | BracedBlock | SquareBracketList | TT.LParen ExprList TT.RParen | TT.BackRef )
			switch ((TT) LA0) {
		...
		LNodeList ErrorTokenList()
		{
			...
			// line 399
			result = LNode.List();
			// Line 400: greedy(TokenListParticle | TT.SingleQuote)*
			for (;;) {
				la0 = (TT) LA0;
		...
		LNodeList TokenListEx()
		{
			TT la0;
			LNodeList result = default(LNodeList);
			// Line 405: ( (TT.Comma|TT.Semicolon) | TT.Newline | TokenListParticle )*
			for (;;) {
				la0 = (TT) LA0;
		...
		LNode TokenListParticle()
		{
			...
			Token lit_lpar = default(Token);
			Token lit_rpar = default(Token);
			// Line 412: ( TT.LParen TokenListEx TT.RParen / SquareBracketList / BracedBlock / TT.Literal / ~(EOF|TT.Comma|TT.Newline|TT.RBrace|TT.RBrack|TT.RParen|TT.Semicolon|TT.SingleQuote) )
			la0 = (TT) LA0;
			if (la0 == TT.LParen) {
				lit_lpar = MatchAny();
				got_TokenListEx = TokenListEx();
		...
		LNode KeywordExpression()
		{
			...
			// Line 435: ((EOF|TT.Newline|TT.RBrace|TT.RBrack|TT.RParen|TT.Semicolon) =>  / Expr (CommaContinuator)*)
			switch ((TT) LA0) {
			...
			// Line 439: greedy((TT.Newline)? BracedBlock)?
			do {
				la0 = (TT) LA0;
				if (la0 == TT.Newline) {
					la1 = (TT) LA(1);
		...
		LNode Continuator(Symbol word)
		{
			...
			// Line 462: greedy(TT.Newline)?
			la0 = (TT) LA0;
			if (la0 == TT.Newline)
				Skip();
			// Line 463: (BracedBlock / TopExpr (greedy(TT.Newline)? BracedBlock / {..}))
			la0 = (TT) LA0;
			if (la0 == TT.LBrace) {
				bb = BracedBlock();
				// line 463
				...
				// Line 465: (greedy(TT.Newline)? BracedBlock / {..})
				do {
					la0 = (TT) LA0;
					if (la0 == TT.Newline) {
						la1 = (TT) LA(1);
