...
namespace LeMP
{
	...
	public class Compiler
	{
		...
		public static MMap<string, Pair<string, string>> KnownOptions = new MMap<string, Pair<string, string>>()
		{
			...
			{ "no-out-header", Pair.Create("", "Don't write the \"// Generated from...\" banner at the top of each output file") },
		};
		...
		public bool ProcessArguments(BMultiMap<string, string> options, bool warnAboutUnknownOptions, IList<string> inputFiles = null)
		{
			...
			if ((flag = ParseBoolOption(options, "no-out-header", sink)) != null)
				NoOutHeader = flag.Value;
		...
		public bool NoOutHeader;        // if true, don't write the "// Generated from..." banner atop output files
		...
		protected virtual void WriteOutput(InputOutput io)
		{
			...
			else
			{
				...
				// UTF-8 without a byte-order mark, to match the Visual Studio "LeMP
				// custom tool" (which writes via Encoding.UTF8.GetBytes, i.e. no BOM).
				using (var stream = File.Open(io.OutFileName, FileMode.Create, FileAccess.Write, FileShare.Read))
				using (var writer = new StreamWriter(stream, new UTF8Encoding(false)))
				{
					if (!NoOutHeader)
						writer.Write(OutputFileHeader(io));
					var str = io.OutPrinter.Print(io.Output, Sink, null, io.OutOptions);
					writer.Write(str);
				}
			}
		...
		/// <summary>Builds the explanatory banner written at the top of each output
		/// file (unless <see cref="NoOutHeader"/> is set). The text is kept identical
		/// to the banner emitted by the Visual Studio LeMP custom tool so that files
		/// regenerated by either path differ only in the embedded version number.</summary>
		protected virtual string OutputFileHeader(InputOutput io)
		{
			string nl = io.OutOptions.NewlineString ?? "\n";
			return string.Format(
				...
				+ "// Use #importMacros to use macros in a given namespace, e.g. #importMacros(Loyc.LLPG);{0}",
				nl, Path.GetFileName(io.FileName),
				typeof(MacroProcessor).Assembly.GetName().Version.ToString());
		}
