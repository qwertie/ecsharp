// Generated from MatchCode.ecs by LeMP custom tool. LeMP version: 2.3.1.0
	...
	partial class StandardMacros
	{
		...
		public static LNode matchCode(LNode node, IMacroContext context)
		{
			...
			if (cmc.ListVars.Count > 0) {
				...
				cmc.ListVars.OrderBy(v => v.Key.Name).Select(kvp => kvp.Value ? LNode.Call(CodeSymbols.Assign, LNode.List(F.Id(kvp.Key), LNode.Call(CodeSymbols.Default, LNode.List(type.SetStyle(NodeStyle.Operator))))).SetStyle(NodeStyle.Operator) : F.Id(kvp.Key)))));
			}
			if (output.Count == 0)
				return ifStmt;
			else {
