// Generated from EcsParserGrammar.les by LeMP custom tool. LeMP version: 2.9.1.0
...
namespace Loyc.Ecs.Parser
{
	...
	partial class EcsParser
	{
		...
		LNode TryParseNonVarDeclExpr(LNodeList attrs, out TentativeResult result)
		{
			...
			{
				var old_10 = _tentativeErrors;	// save old value in uniquely-named variable
		...
		LNode TryParseVarDecl(LNodeList attrs, out TentativeResult result, bool allowUnassigned = false)
		{
			result = new TentativeResult(InputPosition);
			{
				var old_11 = _tentativeErrors;	// save old value in uniquely-named variable
				_tentativeErrors = new TentativeState(true);
				try {
		...
		LNodeList AttributeKeywordsOpt() {
			var attrs = LNode.List();
			AttributeKeywordsOpt(ref attrs);
			return attrs;
		}
		...
		static readonly HashSet<TT> ExpectedAfterTypeAndName_InStmt = new HashSet<TT> { 
			TT.Assign, TT.LBrace, TT.LParen, TT.LBrack, TT.LambdaArrow, TT.Forward, TT.Semicolon, TT.Comma, TT.At	// @{...} token literal (property)
		};
		static readonly HashSet<TT> ExpectedAfterTypeAndName_InExpr = new HashSet<TT> { 
			TT.Assign, TT.LBrace, TT.LambdaArrow, TT.Forward, TT.Semicolon, TT.Comma, TT.EOF, TT.At	// @{...} token literal (property)
		...
		LNodeList TupleTypeList()
		{
			...
			Match((int) TT.Comma);
			// Line 179: (TupleTypeItem (TT.Comma TupleTypeItem)*)?
			switch (LA0) {
			...
			case TT.Operator: case TT.Substitute: case TT.TypeKeyword:
				{
					items.Add(TupleTypeItem());
					// Line 179: (TT.Comma TupleTypeItem)*
					for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							Skip();
		...
		LNode ComplexId(bool declContext = false)
		{
			...
			// Line 192: (TT.ColonColon IdWithOptionalTypeParams)?
			la0 = LA0;
			...
			// Line 196: (TT.Dot IdWithOptionalTypeParams)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Dot) {
					switch (LA(1)) {
		...
		LNode IdAtom()
		{
			// line 208
			LNode r;
			// Line 209: ( TT.Substitute Atom | TT.Operator AnyOperator | (TT.ContextualKeyword|TT.Id|TT.TypeKeyword) | LinqKeywordAsId )
			switch (LA0) {
			case TT.Substitute:
				{
					var t = MatchAny();
		...
		void TParams(bool declContext, ref LNode r)
		{
			...
			// line 236
			int endIndex;
			// Line 237: ( TT.LT TParamDeclOrDataType (TT.Comma TParamDeclOrDataType)* TT.GT | TT.Dot TT.LBrack TT.RBrack | TT.Not TT.LParen TT.RParen | TT.Not IdWithOptionalTypeParams )
			la0 = LA0;
			if (la0 == TT.LT) {
				op = MatchAny();
				list.Add(TParamDeclOrDataType(declContext));
				// Line 238: (TT.Comma TParamDeclOrDataType)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
		...
		{
			LNode result = default(LNode);
			// Line 251: ( DataType / &{declarationContext} NormalAttributesOpt TParamAttributeKeywordsOpt IdAtom / {..} )
			switch (LA0) {
			case TT.ContextualKeyword: case TT.Id: case TT.LinqKeyword: case TT.LParen:
			case TT.Operator: case TT.Substitute: case TT.TypeKeyword:
				result = DataType();
			...
			case TT.AttrKeyword: case TT.In: case TT.LBrack:
				{
					...
					NormalAttributesOpt(ref attrs);
					TParamAttributeKeywordsOpt(ref attrs);
					result = IdAtom();
					// line 257
			...
			case TT.AttrKeyword: case TT.In: case TT.LBrack:
				{
					if (!declarationContext)
						return false;
					if (!Scan_NormalAttributesOpt())
						return false;
					if (!Scan_TParamAttributeKeywordsOpt())
						return false;
					if (!Scan_IdAtom())
						return false;
				}
		...
		LNode ComplexNameDecl(bool thisAllowed, out bool hasThis)
		{
			...
			// line 270
			hasThis = false;
			// Line 271: (TT.Dot ComplexThisDecl)?
			la0 = LA0;
			if (la0 == TT.Dot) {
				la1 = LA(1);
				if (la1 == TT.This) {
		...
		static readonly HashSet<int> MethodOrPropertyName_set0 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.Assign, (int) TT.At, (int) TT.Backslash, (int) TT.BQString, (int) TT.Colon, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.IncDec, (int) TT.LambdaArrow, (int) TT.LEGE, (int) TT.LT, (int) TT.Mul, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Sub, (int) TT.Substitute, (int) TT.XorBits);
		...
		LNode MethodOrPropertyName(bool thisAllowed, out bool hasThis)
		{
			...
			// Line 280: ( ComplexNameDecl | ComplexThisDecl | TT.Operator &{LT($LI).Value `is` @bool} TT.Literal )
			switch (LA0) {
			case TT.Operator:
				{
					la1 = LA(1);
		...
		bool TypeSuffixOpt(TypeParseMode mode, out Token? dimensionBrack, ref LNode e)
		{
				...
				if (la0 == TT.QuestionMark) {
					...
					if ((count = CountDims(LT(0), true)) > 0) {
						...
						if (la1 == TT.RBrack) {
							...
							// Line 358: greedy(&{(count = CountDims(LT($LI), @false)) > 0} TT.LBrack TT.RBrack)*
							for (;;) {
								la0 = LA0;
								if (la0 == TT.LBrack) {
			...
			match1:
				{
					var t = MatchAny();
					// Line 337: (&{mode == TypeParseMode.Normal} | &!(((TT.Add|TT.AndBits|TT.At|TT.ContextualKeyword|TT.Forward|TT.Id|TT.IncDec|TT.LBrace|TT.Literal|TT.LParen|TT.Mul|TT.New|TT.Not|TT.NotBits|TT.Sub|TT.Substitute|TT.TypeKeyword) | LinqKeywordAsId)))
			...
			default:
				{
					...
					// Line 481: greedy(~(EOF|TT.Comma|TT.Semicolon))*
					for (;;) {
						la0 = LA0;
		...
		Token AnyOperator()
		{
			TokenType la0, la1;
			Token result = default(Token);
			// Line 489: (&{LT($LI).EndIndex == LT($LI + 1).StartIndex} (TT.LT TT.LT | TT.GT TT.GT) / (TT.Add|TT.And|TT.AndBits|TT.Assign|TT.At|TT.Backslash|TT.BQString|TT.Colon|TT.ColonColon|TT.Compare|TT.CompoundAssign|TT.DivMod|TT.Dot|TT.DotDot|TT.EqNeq|TT.Forward|TT.GT|TT.IncDec|TT.LambdaArrow|TT.LEGE|TT.LT|TT.Mul|TT.Not|TT.NotBits|TT.NullCoalesce|TT.NullDot|TT.OrBits|TT.OrXor|TT.PipeArrow|TT.Power|TT.PtrArrow|TT.QuestionMark|TT.QuickBind|TT.QuickBindAssign|TT.Sub|TT.Substitute|TT.XorBits))
			la0 = LA0;
			if (la0 == TT.GT || la0 == TT.LT) {
				if (LT(0).EndIndex == LT(0 + 1).StartIndex) {
					la1 = LA(1);
					if (la1 == TT.GT || la1 == TT.LT) {
						// Line 490: (TT.LT TT.LT | TT.GT TT.GT)
						la0 = LA0;
						if (la0 == TT.LT) {
							var op = MatchAny();
							Match((int) TT.LT);
		...
		LNode NewExpr()
		{
			...
			// Line 507: ( &{(count = CountDims(LT($LI), @false)) > 0} TT.LBrack TT.RBrack TT.LBrace TT.RBrace | TT.LBrace TT.RBrace | DataType (TT.LParen TT.RParen (TT.LBrace TT.RBrace)? / (TT.LBrace TT.RBrace)?) )
			la0 = LA0;
			if (la0 == TT.LBrack) {
				...
				// Line 529: (TT.LParen TT.RParen (TT.LBrace TT.RBrace)? / (TT.LBrace TT.RBrace)?)
				do {
					la0 = LA0;
					if (la0 == TT.LParen) {
						la1 = LA(1);
		...
		private LNode TokenLiteral()
		{
			...
			at = Match((int) TT.At);
			// Line 575: (TT.LBrack TT.RBrack | TT.LBrace TT.RBrace)
			la0 = LA0;
			if (la0 == TT.LBrack) {
				L = MatchAny();
		...
		{
			LNode result = default(LNode);
			// Line 581: (&(IdWithOptionalTypeParams ~(TT.ContextualKeyword|TT.Id|TT.LinqKeyword)) IdWithOptionalTypeParams / Atom)
			switch (LA0) {
			case TT.ContextualKeyword: case TT.Id: case TT.LinqKeyword: case TT.Operator:
			case TT.Substitute: case TT.TypeKeyword:
				{
		...
		private LNode PrimaryExpr()
		{
			...
			// Line 589: (TT.NullDot PrimaryExpr)?
			la0 = LA0;
			if (la0 == TT.NullDot) {
				switch (LA(1)) {
		...
		{
			TokenType la1;
			// Line 596: greedy( (TT.ColonColon|TT.Dot|TT.PtrArrow|TT.QuickBind) AtomOrTypeParamExpr / PrimaryExpr_NewStyleCast / TT.LParen TT.RParen | TT.LBrack TT.RBrack | TT.QuestionMark TT.LBrack TT.RBrack | TT.IncDec | BracedBlockOrTokenLiteral )*
			for (;;) {
				switch (LA0) {
		...
		private LNode PrimaryExpr_NewStyleCast(LNode e)
		{
			...
			// Line 638: ( TT.PtrArrow | TT.As | TT.Using )
			la0 = LA0;
			if (la0 == TT.PtrArrow) {
				op = MatchAny();
				// line 638
		...
		{
			TokenType la1;
			// Line 675: ( &{Is($LI, @@await)} TT.ContextualKeyword PrefixExpr / KeywordStmtAsExpr / LinqQueryExpression / PrimaryExpr )
			do {
				switch (LA0) {
		...
		LNode KeywordStmtAsExpr()
		{
			...
			// Line 687: ( ReturnBreakContinueThrow | (GotoCaseStmt / GotoStmt) | SwitchStmt )
			switch (LA0) {
		...
		static readonly HashSet<int> SubExpr_set1 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.AttrKeyword, (int) TT.Base, (int) TT.Break, (int) TT.CheckedOrUnchecked, (int) TT.ContextualKeyword, (int) TT.Continue, (int) TT.Default, (int) TT.Delegate, (int) TT.Dot, (int) TT.DotDot, (int) TT.Forward, (int) TT.Goto, (int) TT.Id, (int) TT.IncDec, (int) TT.Is, (int) TT.LBrace, (int) TT.LinqKeyword, (int) TT.Literal, (int) TT.LParen, (int) TT.Mul, (int) TT.New, (int) TT.Not, (int) TT.NotBits, (int) TT.Operator, (int) TT.Power, (int) TT.Return, (int) TT.Sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.Switch, (int) TT.This, (int) TT.Throw, (int) TT.TypeKeyword, (int) TT.Typeof);
		static readonly HashSet<int> SubExpr_set2 = NewSet((int) TT.Add, (int) TT.AndBits, (int) TT.At, (int) TT.Base, (int) TT.Break, (int) TT.CheckedOrUnchecked, (int) TT.Compare, (int) TT.ContextualKeyword, (int) TT.Continue, (int) TT.Default, (int) TT.Delegate, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.Goto, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.Is, (int) TT.LBrace, (int) TT.LEGE, (int) TT.LinqKeyword, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.New, (int) TT.Not, (int) TT.NotBits, (int) TT.Operator, (int) TT.Power, (int) TT.Return, (int) TT.Sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.Switch, (int) TT.This, (int) TT.Throw, (int) TT.TypeKeyword, (int) TT.Typeof);
		...
		private LNode SubExpr(Precedence context)
		{
			...
			Token lt0;
			var e = PrefixExpr();
			// Line 736: greedy( TT.Switch SwitchBody / &{context.CanParse(prec = InfixPrecedenceOf($LA))} (TT.Add|TT.And|TT.AndBits|TT.BQString|TT.Compare|TT.DivMod|TT.DotDot|TT.EqNeq|TT.GT|TT.In|TT.LEGE|TT.LT|TT.Mul|TT.NotBits|TT.NullCoalesce|TT.OrBits|TT.OrXor|TT.PipeArrow|TT.Power|TT.Sub|TT.Switch|TT.XorBits) SubExpr | &{context.CanParse(prec = EP.Assign)} (TT.Assign|TT.CompoundAssign|TT.QuickBindAssign) AttributeKeywordsOpt SubExpr | &{context.CanParse(prec = EP.Lambda) && !_insidePattern} TT.LambdaArrow SubExpr | &{context.CanParse(prec = EP.Is)} TT.Is Pattern | &{context.CanParse(prec = EP.AsUsing)} (TT.As|TT.Using) DataType | &{context.CanParse(EP.Shift)} &{LT($LI).EndIndex == LT($LI + 1).StartIndex} (TT.LT TT.LT SubExpr | TT.GT TT.GT SubExpr) | &{context.CanParse(EP.IfElse)} TT.QuestionMark SubExpr TT.Colon SubExpr | &{Is($LI, @@with)} &{context.CanParse(prec = EP.Switch)} TT.ContextualKeyword TT.LBrace TT.RBrace | &{((lt0 = LT($LI)).Value == @@when || lt0.Value == @@where && !_insideLinqExpr) && SourceFile.Text.TryGet(lt0.StartIndex, '\0') != '@'} &{context.CanParse(prec = EP.WhenWhere)} TT.ContextualKeyword SubExpr )*
			for (;;) {
				switch (LA0) {
				case TT.Switch:
					{
				...
				case TT.Compare: case TT.DivMod: case TT.DotDot: case TT.EqNeq:
				case TT.In: case TT.LEGE: case TT.Mul: case TT.NotBits:
				case TT.NullCoalesce: case TT.OrBits: case TT.OrXor: case TT.PipeArrow:
				case TT.Power: case TT.Sub: case TT.XorBits:
					{
						la0 = LA0;
						if (context.CanParse(prec = InfixPrecedenceOf(la0))) {
							la1 = LA(1);
				...
				case TT.Assign: case TT.CompoundAssign: case TT.QuickBindAssign:
					{
						if (context.CanParse(prec = EP.Assign)) {
							la1 = LA(1);
							if (SubExpr_set1.Contains((int) la1)) {
								op = MatchAny();
								var attrs = AttributeKeywordsOpt();
								rhs = SubExpr(prec);
								// line 749
								if ((!attrs.IsEmpty)) {
									rhs = rhs.PlusAttrs(attrs).WithRange(attrs[0].Range.StartIndex, rhs.Range.EndIndex);
								}
								// line 750
								e = F.CallInfixOp(e, op, rhs);
							} else
				...
				case TT.Is:
					{
						if (context.CanParse(prec = EP.Is)) {
							la1 = LA(1);
							if (SubExpr_set2.Contains((int) la1)) {
								op = MatchAny();
								rhs = Pattern(prec);
								// line 760
			...
			match7:
				{
					// Line 769: (TT.LT TT.LT SubExpr | TT.GT TT.GT SubExpr)
					la0 = LA0;
					if (la0 == TT.LT) {
						op = MatchAny();
						Match((int) TT.LT);
		...
		public LNode ExprStart(bool allowUnassignedVarDecl)
		{
			TokenType la0, la1;
			LNode result = default(LNode);
			// Line 819: (((TT.ContextualKeyword|TT.Id) | LinqKeywordAsId) TT.Colon ExprStartNNP / ExprStartNNP)
			la0 = LA0;
			if (la0 == TT.ContextualKeyword || la0 == TT.Id || la0 == TT.LinqKeyword) {
				...
				if (la1 == TT.Colon) {
					...
					// Line 820: ((TT.ContextualKeyword|TT.Id) | LinqKeywordAsId)
					la0 = LA0;
		...
		public LNode ExprStartNNP(bool allowUnassignedVarDecl)
		{
			...
			// line 830
			var attrs = LNodeList.Empty;
			var hasList = NormalAttributesOpt(ref attrs);
			AttributeKeywordsOpt(ref attrs);
			// line 835
		...
		private LNode VarDeclExpr(out bool hasInitializer, LNodeList attrs)
		{
			...
			// Line 928: (RestOfPropertyDefinition / VarInitializerOpt)
			switch (LA0) {
		...
		{
			// Line 967: (&(ExprInParens (TT.Assign|TT.LambdaArrow|TT.QuickBindAssign)) ExprInParens / ExprInParens)
		...
		private LNode ExprInParens(bool allowUnassignedVarDecl)
		{
			...
			Token lit_rpar = default(Token);
			// line 975
			var old_12 = _insideLinqExpr;	// save old value in uniquely-named variable
		...
		{
			TokenType la0, la1;
			// Line 987: (EOF => {..} / ExprStart nongreedy(TT.Comma ExprStart)* (TT.Comma)? EOF)
			la0 = LA0;
			if (la0 == EOF)
				// line 989
				...
				// Line 994: nongreedy(TT.Comma ExprStart)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		private LNode BracedBlockOrTokenLiteral(Symbol spaceName = null, Symbol target = null, int startIndex = -1)
		{
			TokenType la0;
			LNode result = default(LNode);
			// Line 1007: (BracedBlock | TokenLiteral)
			la0 = LA0;
			if (la0 == TT.LBrace)
		...
		private LNode BracedBlock(Symbol spaceName = null, Symbol target = null, int startIndex = -1)
		{
			...
			Token lit_rcub = default(Token);
			// line 1012
			var old_13 = _spaceName;	// save old value in uniquely-named variable
		...
		LNode Pattern(Precedence ctx)
		{
			...
			// Line 1230: (&{Is($LI, @@or)} TT.Id ConjunctivePattern)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Id) {
					if (Is(0, sy_or)) {
		...
		LNode ConjunctivePattern(Precedence ctx)
		{
			...
			// Line 1238: greedy(&{Is($LI, @@and)} TT.Id NegatedPattern)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Id) {
					if (Is(0, sy_and)) {
		...
		LNode NegatedPattern(Precedence ctx)
		{
			...
			LNode result = default(LNode);
			// Line 1244: (&{Is($LI, @@not)} TT.Id NegatedPattern / PrimaryPattern)
			la0 = LA0;
			if (la0 == TT.Id) {
				if (Is(0, sy_not)) {
					la1 = LA(1);
		...
		{
			LNode result = default(LNode);
			// Line 1252: ( RelationalPattern / &(RecursivePattern) RecursivePattern / default SubExprInPattern )
			switch (LA0) {
		...
		LNode RecursivePattern()
		{
			...
			// Line 1267: (&{Is($LI, @@var)} TT.ContextualKeyword VarDesignation / DataTypeInPatternOpt greedy(&!{LA(2) == TT.LBrack} TT.LParen TT.RParen (&{type == @null && subpats.Count == 1} &!(IdAtomButNotPatternKeyword) ~(TT.LBrace) => {..} / (~(EOF))? => {..}) (TT.LBrace TT.RBrace)? | TT.LBrace TT.RBrace)? (IdAtomButNotPatternKeyword)?)
			do {
				la0 = LA0;
			...
			matchDataTypeInPatternOpt:
				{
					...
					// Line 1276: greedy(&!{LA(2) == TT.LBrack} TT.LParen TT.RParen (&{type == @null && subpats.Count == 1} &!(IdAtomButNotPatternKeyword) ~(TT.LBrace) => {..} / (~(EOF))? => {..}) (TT.LBrace TT.RBrace)? | TT.LBrace TT.RBrace)?
					la0 = LA0;
					if (la0 == TT.LParen) {
						if (!(LA(2) == TT.LBrack)) {
							la1 = LA(1);
					...
					// Line 1292: (IdAtomButNotPatternKeyword)?
					switch (LA0) {
					case TT.ContextualKeyword: case TT.Id: case TT.LinqKeyword: case TT.Operator:
					case TT.Substitute: case TT.TypeKeyword:
						{
		...
		bool Scan_RecursivePattern()
		{
			...
			// Line 1267: (&{Is($LI, @@var)} TT.ContextualKeyword VarDesignation / DataTypeInPatternOpt greedy(&!{LA(2) == TT.LBrack} TT.LParen TT.RParen  (TT.LBrace TT.RBrace)? | TT.LBrace TT.RBrace)? (IdAtomButNotPatternKeyword)?)
			do {
				la0 = LA0;
				if (la0 == TT.ContextualKeyword) {
					if (Is(0, sy_var)) {
			...
			matchDataTypeInPatternOpt:
				{
					if (!Scan_DataTypeInPattern())
						return false;
					// Line 1276: greedy(&!{LA(2) == TT.LBrack} TT.LParen TT.RParen  (TT.LBrace TT.RBrace)? | TT.LBrace TT.RBrace)?
					la0 = LA0;
					if (la0 == TT.LParen) {
						if (!(LA(2) == TT.LBrack)) {
							la1 = LA(1);
			...
			// Line 1326: ( (TT.Add|TT.BQString|TT.CheckedOrUnchecked|TT.ColonColon|TT.Compare|TT.DivMod|TT.Dot|TT.DotDot|TT.IncDec|TT.LBrack|TT.Literal|TT.LParen|TT.Mul|TT.New|TT.Not|TT.NotBits|TT.Power|TT.PtrArrow|TT.QuickBind|TT.Sub|TT.Switch|TT.Typeof) / (TT.LT TT.LT | TT.GT TT.GT) / default {..} )
			switch (LA0) {
			...
			case TT.GT: case TT.LT:
				{
					la1 = LA(1);
					if (la1 == TT.GT || la1 == TT.LT) {
						// Line 1329: (TT.LT TT.LT | TT.GT TT.GT)
						la0 = LA0;
						if (la0 == TT.LT) {
							Skip();
							if (!TryMatch((int) TT.LT))
		...
		LNode VarDesignation()
		{
			...
			Token lit_rpar = default(Token);
			LNode result = default(LNode);
			// Line 1342: (IdAtom | TT.LParen TT.RParen)
			switch (LA0) {
			case TT.ContextualKeyword: case TT.Id: case TT.LinqKeyword: case TT.Operator:
			case TT.Substitute: case TT.TypeKeyword:
				result = IdAtom();
		...
		LNodeList VarTupleContents()
		{
			...
			// Line 1351: (TT.Comma VarDesignation)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Comma) {
					Skip();
		...
		LNodeList SubPatterns(bool inParens)
		{
			TokenType la0, la1;
			LNodeList result = default(LNodeList);
			// Line 1360: (SubPattern (TT.Comma SubPattern)* (TT.Comma)?)?
			la0 = LA0;
			if (SubExpr_set2.Contains((int) la0)) {
				result.Add(SubPattern());
				// Line 1361: (TT.Comma SubPattern)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		LNode SubPattern()
		{
			...
			LNode result = default(LNode);
			// Line 1371: (TT.Id TT.Colon Pattern / Pattern)
			do {
				la0 = LA0;
				if (la0 == TT.Id) {
					if (Try_Scan_RecursivePattern(0)) {
		...
		LNode SubExprInPattern(Precedence context)
		{
			LNode result = default(LNode);
			// line 1384
			var old_14 = _insidePattern;	// save old value in uniquely-named variable
			_insidePattern = true;
			try {
		...
		LNode DataTypeInPatternOpt()
		{
			TokenType la1, la2;
			LNode result = default(LNode);
			// Line 1401: (TT.LParen TT.RParen (TT.LBrack|TT.QuestionMark) => DataType / ComplexId TypeSuffixOpt)?
			switch (LA0) {
			case TT.LParen:
				{
					la1 = LA(1);
		...
		// makes parsing easier. Speaking of things that make parsing easier, 
		// a recursive pattern syntax of `Type? Id? Tuple? Braces?` would have
		// been easier than their choice of `Type? Tuple? Braces? Id?` (though,
		// thinking on it further, the latter would be less compact sometimes,
		// since `(...) x` would have to be expresed as `var x (...)`. But the
		// ambiguity of `(A<B>.C) x` - cast or variable creation? - disappears)
		LNode SwitchBody()
		{
			Token lit_lcub = default(Token);
			Token lit_rcub = default(Token);
		...
		LNodeList SwitchExpressionArms()
		{
			TokenType la0, la1;
			LNodeList result = default(LNodeList);
			// Line 1459: (SwitchExpressionArm (TT.Comma EOF / TT.Comma SwitchExpressionArm)*)?
			la0 = LA0;
			if (SubExpr_set2.Contains((int) la0)) {
				result.Add(SwitchExpressionArm());
				// Line 1460: (TT.Comma EOF / TT.Comma SwitchExpressionArm)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
						if (la1 == EOF) {
							Skip();
							Skip();
						} else if (SubExpr_set2.Contains((int) la1)) {
							Skip();
							result.Add(SwitchExpressionArm());
						} else
							goto error;
		...
		private void CaseGuard(ref LNode lhs)
		{
			...
			op = MatchAny();
			// line 1473
			var old_15 = _insidePattern;	// save old value in uniquely-named variable
			_insidePattern = true;
			try {
		...
		bool NormalAttributesOpt(ref LNodeList attrs)
		{
			...
			// Line 1484: (&!{Down($LI) && Up(Try_Scan_AsmOrModLabel(0))} TT.LBrack TT.RBrack)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.LBrack) {
		...
		bool Try_Scan_NormalAttributesOpt(int lookaheadAmt) {
			using (new SavePosition(this, lookaheadAmt))
				return Scan_NormalAttributesOpt();
		}
		bool Scan_NormalAttributesOpt()
		{
			TokenType la0, la1;
			// Line 1484: (&!{Down($LI) && Up(Try_Scan_AsmOrModLabel(0))} TT.LBrack TT.RBrack)*
			for (;;) {
		...
		void AttributeContents(ref LNodeList attrs)
		{
			...
			// Line 1497: ((TT.ContextualKeyword|TT.Id|TT.LinqKeyword|TT.Return) TT.Colon ExprList / ExprList)
			switch (LA0) {
		...
		void AttributeKeywordsOpt(ref LNodeList attrs)
		{
			TokenType la0;
			// Line 1524: (TT.AttrKeyword)*
			for (;;) {
				la0 = LA0;
		...
		void TParamAttributeKeywordsOpt(ref LNodeList attrs)
		{
			TokenType la0;
			// Line 1530: ((TT.AttrKeyword|TT.In))*
			for (;;) {
				la0 = LA0;
		...
		bool Try_Scan_TParamAttributeKeywordsOpt(int lookaheadAmt) {
			using (new SavePosition(this, lookaheadAmt))
				return Scan_TParamAttributeKeywordsOpt();
		}
		bool Scan_TParamAttributeKeywordsOpt()
		{
			TokenType la0;
			// Line 1530: ((TT.AttrKeyword|TT.In))*
			for (;;) {
		...
		public LNode LinqQueryExpression()
		{
			...
			var old_16 = _insideLinqExpr;	// save old value in uniquely-named variable
		...
		static readonly HashSet<int> Var_In_Expr_set0 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.Assign, (int) TT.At, (int) TT.Backslash, (int) TT.Base, (int) TT.BQString, (int) TT.CheckedOrUnchecked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.Default, (int) TT.Delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LinqKeyword, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.New, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.Operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.RParen, (int) TT.Sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.This, (int) TT.TypeKeyword, (int) TT.Typeof, (int) TT.XorBits);
		static readonly HashSet<int> Var_In_Expr_set1 = NewSet((int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.Assign, (int) TT.At, (int) TT.Backslash, (int) TT.Base, (int) TT.BQString, (int) TT.CheckedOrUnchecked, (int) TT.Colon, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.Default, (int) TT.Delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.EqNeq, (int) TT.Forward, (int) TT.GT, (int) TT.Id, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LinqKeyword, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.New, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.Operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.This, (int) TT.TypeKeyword, (int) TT.Typeof, (int) TT.XorBits);
		...
		LNode Var_In_Expr()
		{
			...
			LNode result = default(LNode);
			// Line 1558: (&(VarIn) VarIn ExprStart / ExprStart)
			do {
				switch (LA0) {
				case TT.ContextualKeyword: case TT.Id: case TT.LinqKeyword: case TT.LParen:
				case TT.Operator: case TT.Substitute: case TT.TypeKeyword:
		...
		{
			TokenType la0;
			// Line 1567: greedy(QueryBodyClause)*
			for (;;) {
				la0 = LA0;
			...
			// Line 1568: (LinqGroupClause | LinqSelectClause)
			la0 = LA0;
			if (la0 == TT.LinqKeyword) {
				if (Is(0, sy_group))
			...
			// Line 1572: greedy(QueryContinuation)?
			la0 = LA0;
			if (la0 == TT.LinqKeyword) {
				if (Is(0, sy_into))
		...
		private LNode QueryBodyClause()
		{
			TokenType la0;
			LNode result = default(LNode);
			// Line 1577: ( LinqFromClause | LinqLet | LinqWhere | LinqJoin | LinqOrderBy )
			la0 = LA0;
			if (la0 == TT.LinqKeyword) {
				if (Is(0, sy_from))
		...
		private LNode LinqJoin()
		{
			...
			// Line 1608: (&{Is($LI, @@into)} TT.LinqKeyword IdAtom / {..})
			la0 = LA0;
			if (la0 == TT.LinqKeyword) {
				if (Is(0, sy_into)) {
					var intoKw = MatchAny();
		...
		private LNode LinqOrderBy()
		{
			...
			// Line 1621: (TT.Comma LinqOrdering)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Comma) {
					Skip();
		...
		private LNode LinqOrdering()
		{
			...
			LNode result = default(LNode);
			result = ExprStart(false);
			// Line 1628: greedy(&{Is($LI, @@ascending)} TT.LinqKeyword | &{Is($LI, @@descending)} TT.LinqKeyword)?
			la0 = LA0;
			if (la0 == TT.LinqKeyword) {
		...
		private LNode LinqGroupClause()
		{
			...
			kw = MatchAny();
			lhs = ExprStart(false);
			// Line 1642: (&{Is($LI, @@by)} TT.LinqKeyword ExprStart)
			la0 = LA0;
			if (la0 == TT.LinqKeyword) {
		...
		LNode MethodOrPropertyOrVarStmt(int startIndex, LNodeList attrs)
		{
			TokenType la0;
			LNode result = default(LNode);
			// Line 1759: ( TraitDecl / AliasDecl / MethodOrPropertyOrVar )
			la0 = LA0;
			if (la0 == TT.ContextualKeyword) {
				if (Is(0, sy_trait)) {
					switch (LA(1)) {
		...
		LNode KeywordStmt(int startIndex, LNodeList attrs, bool hasWordAttrs)
		{
			...
			// Line 1772: ( ((IfStmt | EventDecl | DelegateDecl | SpaceDecl | EnumDecl | CheckedOrUncheckedStmt | DoStmt | CaseStmt | ReturnBreakContinueThrow TT.Semicolon) | (GotoCaseStmt TT.Semicolon / GotoStmt TT.Semicolon) | SwitchStmt | WhileStmt | ForStmt | ForEachStmt) | (UsingStmt / UsingDirective) | LockStmt | FixedStmt | TryStmt | PPNullaryDirective | PPStringDirective )
			do {
				switch (LA0) {
				case TT.If:
					{
		...
		LNode IdStmt(int startIndex, LNodeList attrs, bool hasWordAttrs)
		{
			...
			bool addAttrs = false;
			string showWordAttrErrorFor = null;
			// Line 1825: ( Constructor / BlockCallStmt / LabelStmt / &(DataType TT.This) DataType => MethodOrPropertyOrVar / ExprStatement )
			do {
				switch (LA0) {
		...
		static readonly HashSet<int> OtherStmt_set0 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.As, (int) TT.Assign, (int) TT.At, (int) TT.Base, (int) TT.BQString, (int) TT.Break, (int) TT.Catch, (int) TT.CheckedOrUnchecked, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.Continue, (int) TT.Default, (int) TT.Delegate, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.Else, (int) TT.EqNeq, (int) TT.Finally, (int) TT.Forward, (int) TT.Goto, (int) TT.GT, (int) TT.Id, (int) TT.In, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LinqKeyword, (int) TT.Literal, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.New, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.Operator, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Return, (int) TT.Semicolon, (int) TT.Sizeof, (int) TT.Sub, (int) TT.Substitute, (int) TT.Switch, (int) TT.This, (int) TT.Throw, (int) TT.TypeKeyword, (int) TT.Typeof, (int) TT.Using, (int) TT.While, (int) TT.XorBits);
		static readonly HashSet<int> OtherStmt_set1 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.As, (int) TT.Assign, (int) TT.At, (int) TT.BQString, (int) TT.Catch, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.Else, (int) TT.EqNeq, (int) TT.Finally, (int) TT.GT, (int) TT.In, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Semicolon, (int) TT.Sub, (int) TT.Switch, (int) TT.Using, (int) TT.While, (int) TT.XorBits);
		static readonly HashSet<int> OtherStmt_set2 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.As, (int) TT.Assign, (int) TT.At, (int) TT.BQString, (int) TT.Catch, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.Else, (int) TT.EqNeq, (int) TT.Finally, (int) TT.GT, (int) TT.In, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Semicolon, (int) TT.Sub, (int) TT.Switch, (int) TT.Using, (int) TT.While, (int) TT.XorBits);
		static readonly HashSet<int> OtherStmt_set3 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.As, (int) TT.Assign, (int) TT.At, (int) TT.BQString, (int) TT.Catch, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.Else, (int) TT.EqNeq, (int) TT.Finally, (int) TT.GT, (int) TT.Id, (int) TT.In, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.Not, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Semicolon, (int) TT.Sub, (int) TT.Substitute, (int) TT.Switch, (int) TT.Using, (int) TT.While, (int) TT.XorBits);
		static readonly HashSet<int> OtherStmt_set4 = NewSet((int) EOF, (int) TT.Add, (int) TT.And, (int) TT.AndBits, (int) TT.As, (int) TT.Assign, (int) TT.At, (int) TT.BQString, (int) TT.Catch, (int) TT.ColonColon, (int) TT.Compare, (int) TT.CompoundAssign, (int) TT.ContextualKeyword, (int) TT.DivMod, (int) TT.Dot, (int) TT.DotDot, (int) TT.Else, (int) TT.EqNeq, (int) TT.Finally, (int) TT.GT, (int) TT.Id, (int) TT.In, (int) TT.IncDec, (int) TT.Is, (int) TT.LambdaArrow, (int) TT.LBrace, (int) TT.LBrack, (int) TT.LEGE, (int) TT.LParen, (int) TT.LT, (int) TT.Mul, (int) TT.NotBits, (int) TT.NullCoalesce, (int) TT.NullDot, (int) TT.OrBits, (int) TT.OrXor, (int) TT.PipeArrow, (int) TT.Power, (int) TT.PtrArrow, (int) TT.QuestionMark, (int) TT.QuickBind, (int) TT.QuickBindAssign, (int) TT.Semicolon, (int) TT.Sub, (int) TT.Substitute, (int) TT.Switch, (int) TT.Using, (int) TT.While, (int) TT.XorBits);
		...
		LNode OtherStmt(int startIndex, LNodeList attrs, bool hasWordAttrs)
		{
			...
			bool addAttrs = false;
			string showWordAttrErrorFor = null;
			// Line 1861: ( BracedBlock / &(TT.NotBits (TT.ContextualKeyword|TT.Id|TT.LinqKeyword|TT.This) TT.LParen TT.RParen TT.LBrace TT.RBrace) Destructor / TT.Semicolon / LabelStmt / default ExprStatement / AssemblyOrModuleAttribute / OperatorCastMethod )
			do {
				switch (LA0) {
				case TT.LBrace:
					{
				...
				case TT.Operator:
					{
						switch (LA(1)) {
						case TT.Add: case TT.And: case TT.AndBits: case TT.Assign:
						case TT.At: case TT.Backslash: case TT.BQString: case TT.Colon:
						case TT.ColonColon: case TT.Compare: case TT.CompoundAssign: case TT.DivMod:
						case TT.Dot: case TT.DotDot: case TT.EqNeq: case TT.Forward:
						case TT.GT: case TT.IncDec: case TT.LambdaArrow: case TT.LEGE:
						case TT.LT: case TT.Mul: case TT.Not: case TT.NotBits:
						case TT.NullCoalesce: case TT.NullDot: case TT.OrBits: case TT.OrXor:
						case TT.PipeArrow: case TT.Power: case TT.PtrArrow: case TT.QuestionMark:
						case TT.QuickBind: case TT.QuickBindAssign: case TT.Sub: case TT.Substitute:
						case TT.XorBits:
							goto matchExprStatement;
						case TT.ContextualKeyword: case TT.Id: case TT.LinqKeyword: case TT.LParen:
		...
		LNode ExprStatement()
		{
			...
			// Line 1895: ((EOF|TT.Catch|TT.Else|TT.Finally|TT.While) =>  | TT.Semicolon)
			switch (LA0) {
		...
		{
			TokenType la0;
			// Line 1906: greedy(~(EOF|TT.LBrace|TT.Semicolon))*
			for (;;) {
				la0 = LA0;
			...
			// Line 1907: greedy(TT.Semicolon | TT.LBrace (TT.RBrace)?)?
			la0 = LA0;
			if (la0 == TT.Semicolon)
				Skip();
			else if (la0 == TT.LBrace) {
		...
		private LNode RestOfSpaceDecl(int startIndex, Token kindTok)
		{
			...
			// Line 1931: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				// line 1932
		...
		LNode AliasDecl(int startIndex)
		{
			...
			Match((int) TT.Assign, (int) TT.QuickBindAssign);
		...
		LNode UsingDirective(int startIndex, LNodeList attrs)
		{
			...
			// Line 1950: (&{Is($LI, S.Static)} TT.AttrKeyword ExprStart TT.Semicolon / ExprStart (&{nsName.Calls(S.Assign, 2)} RestOfAlias / TT.Semicolon))
			do {
				la0 = LA0;
				if (la0 == TT.AttrKeyword) {
					if (Is(0, S.Static)) {
			...
			matchExprStart:
				{
					nsName = ExprStart(true);
					// Line 1955: (&{nsName.Calls(S.Assign, 2)} RestOfAlias / TT.Semicolon)
					do {
						switch (LA0) {
						case TT.Semicolon:
							{
		...
		LNode RestOfAlias(int startIndex, Token aliasTok, LNode oldName, LNode newName)
		{
			...
			// Line 1971: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				// line 1972
		...
		private LNode EnumDecl(int startIndex)
		{
			...
			var name = ComplexNameDecl();
			var bases = BaseListOpt();
			// Line 1983: (TT.Semicolon | TT.LBrace TT.RBrace)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
				// line 1984
		...
		{
			TokenType la0;
			// Line 1995: (TT.Colon DataType (TT.Comma DataType)* | {..})
			la0 = LA0;
			if (la0 == TT.Colon) {
				// line 1995
		...
		private KeyValuePair<Symbol, LNode> WhereClause()
		{
			...
			// Line 2039: (TT.Comma WhereConstraint)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Comma) {
					Skip();
		...
		{
			TokenType la0;
			// Line 2044: ( (TT.Class|TT.Struct) | TT.New &{LT($LI).Count == 0} TT.LParen TT.RParen | DataType )
			la0 = LA0;
		...
		private LNode MethodOrPropertyOrVar(int startIndex, LNodeList attrs)
		{
			...
			// Line 2086: ( &{!isNamedThis} MethodArgListAndBody | &!{name.IsLiteral} RestOfPropertyDefinition | &{!isNamedThis} &!{name.IsLiteral} VarInitializerOpt (TT.Comma ComplexNameDecl VarInitializerOpt)* TT.Semicolon )
			switch (LA0) {
			case TT.LParen:
				{
			...
			case TT.LBrace: case TT.LBrack:
				...
				break;
			case TT.Assign: case TT.Comma: case TT.QuickBindAssign: case TT.Semicolon:
				{
					Check(!isNamedThis, "Expected !isNamedThis");
					...
					// Line 2099: (TT.Comma ComplexNameDecl VarInitializerOpt)*
					for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							Skip();
		...
		private LNode VarInitializerOpt(LNode name, bool isArray)
		{
			...
			// Line 2112: (VarInitializer)?
			la0 = LA0;
			if (la0 == TT.Assign || la0 == TT.QuickBindAssign) {
				var eq = LT0;
		...
		private LNode VarInitializer(bool isArray)
		{
			...
			Skip();
			var attrs = AttributeKeywordsOpt();
			// Line 2123: (&{isArray} &{Down($LI) && Up(HasNoSemicolons())} TT.LBrace TT.RBrace / ExprStart)
			la0 = LA0;
			if (la0 == TT.LBrace) {
				if (isArray) {
			...
			// line 2133
			if ((!attrs.IsEmpty)) {
				result = result.PlusAttrs(attrs).WithRange(attrs[0].Range.StartIndex, result.Range.EndIndex);
			}
			return result;
		}
		...
		private LNode MethodArgListAndBody(int startIndex, int targetIndex, LNodeList attrs, Symbol kind, LNode type, LNode name)
		{
			...
			// line 2164
			int consCallIndex = -1;
			// Line 2165: (TT.Colon (TT.Base|TT.This) TT.LParen TT.RParen)?
			la0 = LA0;
			if (la0 == TT.Colon) {
				lit_colon = MatchAny();
			...
			// Line 2184: (default TT.Semicolon | MethodBodyOrForward)
			do {
				switch (LA0) {
				case TT.Semicolon:
					goto match1;
		...
		private LNode MethodBodyOrForward(bool isProperty, out LNode propInitializer, bool isExpression = false, int bodyStartIndex = -1)
		{
			...
			// Line 2215: ( TT.Forward ExprStart SemicolonIf | TT.LambdaArrow ExprStart SemicolonIf | TokenLiteral (&{!isExpression} TT.Semicolon)? | BracedBlock greedy(&{isProperty} TT.Assign ExprStart SemicolonIf)? )
			la0 = LA0;
			if (la0 == TT.Forward) {
				var op = MatchAny();
				...
				var e = TokenLiteral();
				// Line 2217: (&{!isExpression} TT.Semicolon)?
				la0 = LA0;
				if (la0 == TT.Semicolon) {
				...
				// Line 2221: greedy(&{isProperty} TT.Assign ExprStart SemicolonIf)?
				la0 = LA0;
				if (la0 == TT.Assign) {
		...
		{
			TokenType la0;
			// Line 2229: (&{isStatement} TT.Semicolon / {..})
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				if (isStatement)
					Skip();
		...
		private LNode Constructor(int startIndex, LNodeList attrs)
		{
			...
			LNode r;
			Token n;
			// Line 2259: ( &{_spaceName == LT($LI).Value} (TT.ContextualKeyword|TT.Id|TT.LinqKeyword) &(TT.LParen TT.RParen (TT.LBrace|TT.Semicolon)) / &{_spaceName != S.Fn || LA($LI + 3) == TT.LBrace} TT.This &(TT.LParen TT.RParen (TT.LBrace|TT.Semicolon)) / (TT.ContextualKeyword|TT.Id|TT.LinqKeyword|TT.This) &(TT.LParen TT.RParen TT.Colon) )
			do {
				la0 = LA0;
				if (la0 == TT.ContextualKeyword || la0 == TT.Id || la0 == TT.LinqKeyword) {
					if (_spaceName == LT(0).Value) {
		...
		private LNode EventDecl(int startIndex)
		{
			...
			var type = DataType();
			var name = ComplexNameDecl();
			// Line 2298: (TT.Comma ComplexNameDecl (TT.Comma ComplexNameDecl)*)?
			la0 = LA0;
			if (la0 == TT.Comma) {
				// line 2298
				...
				// Line 2299: (TT.Comma ComplexNameDecl)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
			...
			// Line 2302: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				lit_semi = MatchAny();
				// line 2303
		...
		LNode CaseStmt(int startIndex)
		{
			...
			// Line 2324: (TT.Comma ExprStartNNP)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Comma) {
					Skip();
		...
		private LNode BlockCallStmt(int startIndex)
		{
			...
			// line 2342
			LNode block;
			// Line 2343: ( TT.LParen TT.RParen (BracedBlock | TT.Id => Stmt) | TT.Forward ExprStart TT.Semicolon | BracedBlock )
			la0 = LA0;
			if (la0 == TT.LParen) {
				var lp = MatchAny();
				var rp = Match((int) TT.RParen);
				...
				// Line 2344: (BracedBlock | TT.Id => Stmt)
				la0 = LA0;
				if (la0 == TT.LBrace)
					block = BracedBlock();
				else {
		...
		private LNode GotoStmt(int startIndex)
		{
			TokenType la0;
			var kw = MatchAny();
			// Line 2390: (TT.Default / ExprOrNull)
			la0 = LA0;
			if (la0 == TT.Default) {
				var def = MatchAny();
				// line 2391
		...
		private LNode GotoCaseStmt(int startIndex)
		{
			...
			var kw = MatchAny();
			var kw2 = MatchAny();
			// Line 2404: (TT.Default / ExprStartNNP)
			la0 = LA0;
			if (la0 == TT.Default) {
				var def = MatchAny();
				// line 2405
		...
		private LNode TryStmt(int startIndex)
		{
			...
			LNode varExpr;
			LNode whenExpr;
			// Line 2531: greedy(TT.Catch (TT.LParen TT.RParen / {..}) (&{Is($LI, @@when)} TT.ContextualKeyword TT.LParen TT.RParen / {..}) Stmt)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Catch) {
					var kw = MatchAny();
					// Line 2532: (TT.LParen TT.RParen / {..})
					la0 = LA0;
					if (la0 == TT.LParen) {
						la1 = LA(1);
						if (la1 == TT.RParen) {
						...
						// line 2533
						varExpr = MissingHere();
					// Line 2534: (&{Is($LI, @@when)} TT.ContextualKeyword TT.LParen TT.RParen / {..})
					la0 = LA0;
					if (la0 == TT.ContextualKeyword) {
						if (Is(0, sy_when)) {
							la1 = LA(1);
			...
			// Line 2541: greedy(TT.Finally Stmt)*
			for (;;) {
				la0 = LA0;
				if (la0 == TT.Finally) {
					var kw = MatchAny();
		...
		{
			TokenType la0, la1;
			// Line 2593: nongreedy(ExprOpt (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF || la0 == TT.Semicolon)
				;
			else {
				list.Add(ExprOpt(allowUnassignedVarDecl));
				// Line 2594: (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			TokenType la0;
			// Line 2605: nongreedy(ExprOpt (TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(ExprOpt(true));
				// Line 2606: (TT.Comma ExprOpt)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
		...
		LNode InitializerExpr()
		{
			...
			LNode result = default(LNode);
			// Line 2614: ( TT.LBrace TT.RBrace / TT.LBrack TT.RBrack TT.Assign ExprStart / ExprOpt )
			la0 = LA0;
			if (la0 == TT.LBrace) {
				la2 = LA(2);
		...
		{
			TokenType la0, la1;
			// Line 2628: nongreedy(InitializerExpr (TT.Comma EOF / TT.Comma InitializerExpr)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(InitializerExpr());
				// Line 2629: (TT.Comma EOF / TT.Comma InitializerExpr)*
				for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			// Line 338: ((TT.Add|TT.AndBits|TT.At|TT.ContextualKeyword|TT.Forward|TT.Id|TT.IncDec|TT.LBrace|TT.Literal|TT.LParen|TT.Mul|TT.New|TT.Not|TT.NotBits|TT.Sub|TT.Substitute|TT.TypeKeyword) | LinqKeywordAsId)
			switch (LA0) {
		...
		{
			// Line 666: ( (TT.Add|TT.AndBits|TT.BQString|TT.Dot|TT.Mul|TT.Sub|TT.Switch) | TT.IncDec TT.LParen | &{_insideLinqExpr} TT.LinqKeyword )
			switch (LA0) {
		...
		private bool ExprInParensAuto_Test0()
		{
			if (!Scan_ExprInParens(true))
				return false;
			if (!TryMatch((int) TT.Assign, (int) TT.LambdaArrow, (int) TT.QuickBindAssign))
				return false;
			return true;
		}
		...
		{
			TokenType la0;
			// Line 2340: ( TT.LParen TT.RParen (TT.LBrace TT.RBrace | TT.Id) | TT.LBrace TT.RBrace | TT.Forward )
			la0 = LA0;
			if (la0 == TT.LParen) {
				Skip();
				if (!TryMatch((int) TT.RParen))
					return false;
				// Line 2340: (TT.LBrace TT.RBrace | TT.Id)
				la0 = LA0;
				if (la0 == TT.LBrace) {
					Skip();
					if (!TryMatch((int) TT.RBrace))
