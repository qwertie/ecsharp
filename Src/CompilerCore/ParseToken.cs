...
namespace Loyc.CompilerCore
{
	...
	public class ParseToken
	{
		...
		static private CompilerMsg ParseString2(ICharSource source, ref int pos, int endAt, char quote, string tqIndent, bool verbatim, StringBuilder sb)
		{
			...
			char c;
			int i;
			for (i = pos; (c = source[i]) != EOF && i < endAt;) {
				if (c == '\\' && !verbatim)
				...
				} else // c == quote
				{
					...
					else if (source[i + 1] == quote && source[i + 2] == quote)
					{
						pos = i + 3;
		...
		{
			char quote = source[pos], c;
			if ((quote == '"' || quote == '\'') && source[pos + 2] == quote &&
				(c = source[pos + 1]) != '\\' && !IsNEWLINE_CHAR(c) && c != quote)
