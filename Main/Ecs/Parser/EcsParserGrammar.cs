// Generated from EcsParserGrammar.les by LLLPG custom tool. LLLPG version: 1.1.0.0
...
namespace Ecs.Parser
{
	...
	partial class EcsParser
	{
		...
		bool TypeSuffixOpt(bool afterAsOrIs, out Token? dimensionBrack, ref LNode e)
		{
			...
			bool result = false;
			dimensionBrack = null;
			// Line 241: greedy( TT.QuestionMark (&!{afterAsOrIs} | &!(((TT.Not|TT.Id|TT.OtherLit|TT.Symbol|TT.NotBits|TT.At|TT.LParen|TT.Mul|TT.TypeKeyword|TT.Add|TT.LBrace|TT.Sub|TT.Forward|TT.IncDec|TT.String|TT.SQString|TT.AndBits|TT.Number|TT.@new|TT.Substitute) | UnusualId))) | TT.Mul | &{(count = CountDims(LT($LI), @true)) > 0} TT.LBrack TT.RBrack greedy(&{(count = CountDims(LT($LI), @false)) > 0} TT.LBrack TT.RBrack)* )*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.QuestionMark) {
					if (!afterAsOrIs || !Try_TypeSuffixOpt_Test0(1) && (count = CountDims(LT(1), true)) > 0) {
						var t = MatchAny();
						// Line 241: (&!{afterAsOrIs} | &!(((TT.Not|TT.Id|TT.OtherLit|TT.Symbol|TT.NotBits|TT.At|TT.LParen|TT.Mul|TT.TypeKeyword|TT.Add|TT.LBrace|TT.Sub|TT.Forward|TT.IncDec|TT.String|TT.SQString|TT.AndBits|TT.Number|TT.@new|TT.Substitute) | UnusualId)))
						if (!afterAsOrIs) {
						} else
							Check(!Try_TypeSuffixOpt_Test0(0), "!(((TT.Not|TT.Id|TT.OtherLit|TT.Symbol|TT.NotBits|TT.At|TT.LParen|TT.Mul|TT.TypeKeyword|TT.Add|TT.LBrace|TT.Sub|TT.Forward|TT.IncDec|TT.String|TT.SQString|TT.AndBits|TT.Number|TT.@new|TT.Substitute) | UnusualId))");
		...
		LNode Atom()
		{
			TokenType la0, la1;
			LNode r;
			// Line 377: ( (TT.Dot|TT.Substitute) Atom | TT.@operator AnyOperator | (TT.Id|@`.`(TT, noMacro(@this))|TT.TypeKeyword|@`.`(TT, noMacro(@base))) | UnusualId | (TT.SQString|TT.OtherLit|TT.Symbol|TT.Number|TT.String) | ExprInParensAuto | BracedBlock | NewExpr | TT.At TT.LBrack TT.RBrack | (TT.@checked|TT.@unchecked) TT.LParen TT.RParen | (TT.@typeof|@`.`(TT, noMacro(@default))|TT.@sizeof) TT.LParen TT.RParen | TT.@delegate TT.LParen TT.RParen TT.LBrace TT.RBrace )
			 switch (LA0) {
			case TT.Dot:
			case TT.Substitute:
				{
			...
			default:
				{
					...
					// Line 399: greedy(~(TT.Comma|TT.Semicolon|EOF))*
					 for (;;) {
						la0 = LA0;
		...
		{
			// Line 497: (&(ExprInParens (TT.Set|TT.LambdaArrow)) ExprInParens / ExprInParens)
		...
		LNode PrimaryExpr()
		{
			TokenType la1;
			var e = Atom();
			// Line 506: greedy( (TT.QuickBind|TT.Dot|TT.PtrArrow|TT.ColonColon) Atom / PrimaryExpr_NewStyleCast / TT.LParen TT.RParen | TT.At TT.LBrace TT.RBrace | TT.LBrack TT.RBrack | TT.IncDec | &(TParams ~(TT.Id|TT.ContextualKeyword)) ((TT.LT|TT.Not) | TT.Dot TT.LBrack) => TParams | BracedBlock )*
			 for (;;) {
				switch (LA0) {
				case TT.Dot:
					{
		...
		{
			TokenType la2;
			// Line 563: ( (TT.AndBits|TT.NotBits|TT.Mul|TT.Add|TT.Sub|TT.IncDec|TT.Forward|TT.Not) PrefixExpr | (&{Down($LI) && Up(Scan_DataType() && LA0 == EOF)} TT.LParen TT.RParen &!(((TT.Add|TT.Sub) | TT.IncDec TT.LParen)) PrefixExpr / TT.Power PrefixExpr / NullDotExpr) )
			 do {
				switch (LA0) {
				case TT.Add:
				case TT.AndBits:
		...
		LNode Expr(Precedence context)
		{
			...
			Precedence prec;
			var e = PrefixExpr();
			// Line 624: greedy( &{context.CanParse(prec = InfixPrecedenceOf($LA))} (TT.AndBits|TT.NotBits|TT.EqNeq|TT.GT|TT.Mul|TT.LEGE|TT.Power|TT.Add|TT.Sub|TT.NullCoalesce|TT.OrXor|TT.LambdaArrow|TT.Set|TT.XorBits|TT.CompoundSet|TT.OrBits|TT.BQString|TT.LT|TT.DivMod|TT.DotDot|TT.@in|TT.And) Expr | &{context.CanParse(prec = InfixPrecedenceOf($LA))} (TT.@as|TT.@is|TT.@using) DataType | &{context.CanParse(EP.Shift)} &{LT($LI).EndIndex == LT($LI + 1).StartIndex} (TT.LT TT.LT Expr | TT.GT TT.GT Expr) | &{context.CanParse(EP.IfElse)} TT.QuestionMark Expr TT.Colon Expr )*
			 for (;;) {
				switch (LA0) {
				case TT.GT:
				case TT.LT:
		...
		void DetectVarDecl(bool allowUnassigned)
		{
			TokenType la0;
			VarDeclStart();
			// Line 688: ((TT.Set|TT.QuickBindSet) NoUnmatchedColon / &{allowUnassigned} (TT.Comma|EOF))
			la0 = LA0;
		...
		{
			TokenType la0;
			// Line 706: (SubConditional | ~(TT.Comma|TT.Semicolon|TT.Colon|EOF|TT.QuestionMark))*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.QuestionMark)
					SubConditional();
		...
		void SubConditional()
		{
			TokenType la0;
			Skip();
			// Line 711: nongreedy(SubConditional / ~(TT.Comma|TT.Semicolon|EOF))*
			 for (;;) {
				la0 = LA0;
		...
		LNode VarDeclExpr()
		{
			...
			// Line 716: ((TT.Set|TT.QuickBindSet) Expr)?
			la0 = LA0;
			if (la0 == TT.QuickBindSet || la0 == TT.Set) {
				Skip();
		...
		{
			TokenType la0, la1;
			// Line 757: (ExprStart (TT.Comma (~(EOF))* => (TT.Comma ExprStart | TT.Comma)*)? | )
			la0 = LA0;
			if (InParens_ExprOrTuple_set0.Contains((int) la0)) {
				var e = ExprStart(allowUnassignedVarDecl);
				// Line 758: (TT.Comma (~(EOF))* => (TT.Comma ExprStart | TT.Comma)*)?
				la0 = LA0;
				if (la0 == TT.Comma) {
					var list = new RVList<LNode> { 
						e
					};
					// Line 760: (TT.Comma ExprStart | TT.Comma)*
					 for (;;) {
						la0 = LA0;
						if (la0 == TT.Comma) {
							la1 = LA(1);
		...
		{
			TokenType la0, la1;
			// Line 786: (&!{Down($LI) && Up(Try_Scan_AsmOrModLabel(0))} TT.LBrack TT.RBrack)*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.LBrack) {
		...
		void AttributeContents(ref RWList<LNode> attrs)
		{
			...
			// Line 799: greedy((TT.Id|TT.ContextualKeyword|@`.`(TT, noMacro(@return))) TT.Colon)?
			la0 = LA0;
		...
		int WordAttributes(ref RWList<LNode> attrs)
		{
			...
			if (LA0 == TT.Id && ((LA1 = LA(1)) == TT.Set || LA1 == TT.LParen || LA1 == TT.Dot))
				 return 0;
			Token t;
			// Line 902: (TT.AttrKeyword | ((TT.Id|@`.`(TT, noMacro(@this))|TT.@new) | UnusualId) &(( DataType ((TT.Id|TT.AttrKeyword|TT.TypeKeyword) | UnusualId) | (TT.AttrKeyword|TT.@new) | &{_spaceName != S.Def} @`.`(TT, noMacro(@this)) | TT.@checked TT.LBrace TT.RBrace | TT.@unchecked TT.LBrace TT.RBrace | @`.`(TT, noMacro(@default)) TT.Colon | TT.@using TT.LParen | (TT.@while|TT.@try|TT.@switch|TT.@interface|@`.`(TT, noMacro(@throw))|TT.@namespace|TT.@delegate|TT.@for|@`.`(TT, noMacro(@return))|TT.@do|TT.@event|TT.@foreach|TT.@struct|TT.@fixed|TT.@goto|TT.@lock|TT.@case|TT.@class|@`.`(TT, noMacro(@break))|TT.@enum|@`.`(TT, noMacro(@continue))) )))*
			 for (;;) {
				switch (LA0) {
				case TT.AttrKeyword:
					{
				...
				case TT.Id:
					{
						if (Try_WordAttributes_Test0(1)) {
							// Line 903: ((TT.Id|@`.`(TT, noMacro(@this))|TT.@new) | UnusualId)
							la0 = LA0;
		...
		public LNode Stmt()
		{
			...
			LNode r;
			// Line 982: ( (UsingDirective | AssemblyOrModuleAttribute | EventDecl | DelegateDecl | SpaceDecl | EnumDecl) | (TraitDecl / AliasDecl / &(DataType ComplexNameDecl (TT.Comma|TT.Set|TT.Semicolon|TT.At|TT.LParen|TT.LBrace|TT.QuickBindSet|TT.Forward|TT.LambdaArrow)) MethodOrPropertyOrVar / OperatorCast / Constructor / Destructor / BracedBlock / BlockCallStmt / ExprStatement) | CheckedOrUncheckedStmt | DoStmt | CaseStmt | GotoStmt | GotoCaseStmt | LabelStmt | ReturnBreakContinueThrow | WhileStmt | ForStmt | ForEachStmt | IfStmt | SwitchStmt | UsingStmt | LockStmt | FixedStmt | TryStmt | TT.Semicolon )
			 do {
				switch (LA0) {
				case TT.@using:
					{
		...
		LNode ExprStatement()
		{
			...
			// Line 1033: ((TT.@else|EOF|TT.@catch|TT.@finally|TT.@while) =>  | default TT.Semicolon)
			 switch (LA0) {
			case EOF:
			case TT.@catch:
			case TT.@else:
		...
		{
			TokenType la0;
			// Line 1042: greedy(~(TT.Semicolon|EOF|TT.LBrace))*
			 for (;;) {
				la0 = LA0;
			...
			// Line 1043: greedy(TT.Semicolon | TT.LBrace (TT.RBrace)?)?
			la0 = LA0;
			if (la0 == TT.Semicolon)
				Skip();
			else if (la0 == TT.LBrace) {
		...
		LNode RestOfSpaceDecl(int startIndex, Symbol kind, RVList<LNode> attrs)
		{
			...
			// Line 1067: (TT.Semicolon | BracedBlock)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
		...
		LNode UsingDirective(int startIndex, RVList<LNode> attrs)
		{
			...
			// Line 1084: (RestOfAlias | TT.Semicolon)
			la0 = LA0;
			if (la0 == TT.QuickBindSet || la0 == TT.Set) {
		...
		LNode EnumDecl(int startIndex, RVList<LNode> attrs)
		{
			...
			var name = IdNode(id);
			var bases = BaseListOpt();
			// Line 1107: (TT.Semicolon | TT.LBrace TT.RBrace)
			la0 = LA0;
			if (la0 == TT.Semicolon) {
				var end = MatchAny();
		...
		{
			TokenType la0;
			// Line 1119: (TT.Colon DataType (TT.Comma DataType)* | )
			la0 = LA0;
			if (la0 == TT.Colon) {
				var bases = new RVList<LNode>();
				Skip();
				bases.Add(DataType());
				// Line 1121: (TT.Comma DataType)*
				 for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
		...
		KeyValuePair<Symbol,LNode> WhereClause()
		{
			...
			// Line 1174: (TT.Comma WhereConstraint)*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.Comma) {
					Skip();
		...
		{
			TokenType la0;
			// Line 1179: ( (TT.@class|TT.@struct) | TT.@new &{LT($LI).Count == 0} TT.LParen TT.RParen | DataType )
			la0 = LA0;
		...
		LNode MethodOrPropertyOrVar(int startIndex, RVList<LNode> attrs)
		{
			...
			LNode r;
			var type = DataType();
			// Line 1223: (NameAndMaybeInit (TT.Comma NameAndMaybeInit)* TT.Semicolon / ComplexNameDecl (MethodArgListAndBody | WhereClausesOpt MethodBodyOrForward))
			 do {
				la0 = LA0;
				if (la0 == TT.ContextualKeyword || la0 == TT.Id) {
					switch (LA(1)) {
					...
					case TT.Set:
						{
							...
							// Line 1226: (TT.Comma NameAndMaybeInit)*
							 for (;;) {
								la0 = LA0;
								if (la0 == TT.Comma) {
									Skip();
			...
			match2:
				{
					var name = ComplexNameDecl();
					// Line 1230: (MethodArgListAndBody | WhereClausesOpt MethodBodyOrForward)
					 switch (LA0) {
					case TT.LParen:
		...
		LNode MethodArgListAndBody(int startIndex, RVList<LNode> attrs, Symbol kind, LNode type, LNode name)
		{
			...
			// Line 1255: (TT.Colon (@`.`(TT, noMacro(@this))|@`.`(TT, noMacro(@base))) TT.LParen TT.RParen)?
			la0 = LA0;
			if (la0 == TT.Colon) {
				Skip();
			...
			// Line 1272: (default TT.Semicolon | MethodBodyOrForward)
			 do {
				switch (LA0) {
				case TT.Semicolon:
					goto match1;
		...
		{
			TokenType la0;
			// Line 1295: ( TT.Forward ExprStart TT.Semicolon | TT.LambdaArrow ExprStart TT.Semicolon | TT.At TT.LBrack TT.RBrack TT.Semicolon | BracedBlock )
			la0 = LA0;
			if (la0 == TT.Forward) {
				var op = MatchAny();
		...
		LNode NameAndMaybeInit(bool isArray)
		{
			...
			// Line 1320: ((TT.Set|TT.QuickBindSet) (&{isArray} &{Down($LI) && Up(HasNoSemicolons())} TT.LBrace TT.RBrace / ExprStart))?
			la0 = LA0;
			if (la0 == TT.QuickBindSet || la0 == TT.Set) {
				Skip();
		...
		LNode Constructor(int startIndex, RVList<LNode> attrs)
		{
			...
			LNode r;
			Token n;
			// Line 1349: ( &{_spaceName == LT($LI).Value} (TT.Id|TT.ContextualKeyword) &(TT.LParen TT.RParen (TT.Semicolon|TT.LBrace)) / &{_spaceName != S.Def} @`.`(TT, noMacro(@this)) &(TT.LParen TT.RParen (TT.Semicolon|TT.LBrace)) / (TT.Id|@`.`(TT, noMacro(@this))|TT.ContextualKeyword) &(TT.LParen TT.RParen TT.Colon) )
			 do {
				la0 = LA0;
				if (la0 == TT.ContextualKeyword || la0 == TT.Id) {
					if (_spaceName == LT(0).Value) {
		...
		LNode Destructor(int startIndex, RVList<LNode> attrs)
		{
			...
			Token n;
			var tilde = MatchAny();
			// Line 1366: ((&{LT($LI).Value == _spaceName}) (TT.Id|TT.ContextualKeyword) | @`.`(TT, noMacro(@this)))
			la0 = LA0;
			if (la0 == TT.ContextualKeyword || la0 == TT.Id) {
		...
		LNode BlockCallStmt()
		{
			...
			LNode block;
			// Line 1429: ( TT.LParen TT.RParen (BracedBlock | TT.Id => Stmt) | TT.Forward ExprStart TT.Semicolon | BracedBlock )
			la0 = LA0;
			if (la0 == TT.LParen) {
				var lp = MatchAny();
				var rp = Match((int) TT.RParen);
				AppendExprsInside(lp, args);
				// Line 1430: (BracedBlock | TT.Id => Stmt)
				la0 = LA0;
				if (la0 == TT.LBrace)
					block = BracedBlock();
				else {
		...
		LNode GotoCaseStmt(int startIndex)
		{
			...
			Skip();
			Skip();
			// Line 1479: (@`.`(TT, noMacro(@default)) | ExprOpt)
			la0 = LA0;
			if (la0 == TT.@default) {
				la1 = LA(1);
				if (la1 == TT.Semicolon) {
		...
		LNode VarIn()
		{
			TokenType la1;
			LNode @var;
			// Line 1549: (&(Atom TT.@in) Atom / VarDeclStart)
			 do {
				switch (LA0) {
				case TT.@operator:
				case TT.ContextualKeyword:
		...
		LNode TryStmt(int startIndex)
		{
			...
			LNode expr, handler;
			// Line 1612: greedy(TT.@catch (TT.LParen TT.RParen Stmt / Stmt))*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.@catch) {
					la1 = LA(1);
					if (IfStmt_set0.Contains((int) la1)) {
						var kw = MatchAny();
						// Line 1613: (TT.LParen TT.RParen Stmt / Stmt)
						la0 = LA0;
						if (la0 == TT.LParen) {
							var p = MatchAny();
							Match((int) TT.RParen);
			...
			// Line 1619: greedy(TT.@finally Stmt)*
			 for (;;) {
				la0 = LA0;
				if (la0 == TT.@finally) {
					la1 = LA(1);
		...
		{
			TokenType la0, la1;
			// Line 1649: nongreedy(ExprOpt (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(ExprOpt(allowUnassignedVarDecl));
				// Line 1650: (TT.Comma &{allowTrailingComma} EOF / TT.Comma ExprOpt)*
				 for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			TokenType la0;
			// Line 1658: nongreedy(ExprOpt (TT.Comma ExprOpt)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(ExprOpt(true));
				// Line 1659: (TT.Comma ExprOpt)*
				 for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						Skip();
		...
		LNode InitializerExpr()
		{
			TokenType la0;
			LNode e;
			// Line 1667: (TT.LBrace TT.RBrace / ExprOpt)
			la0 = LA0;
			if (la0 == TT.LBrace) {
				var lb = MatchAny();
				var rb = Match((int) TT.RBrace);
		...
		{
			TokenType la0, la1;
			// Line 1678: nongreedy(InitializerExpr (TT.Comma EOF / TT.Comma InitializerExpr)*)?
			la0 = LA0;
			if (la0 == EOF)
				;
			else {
				list.Add(InitializerExpr());
				// Line 1679: (TT.Comma EOF / TT.Comma InitializerExpr)*
				 for (;;) {
					la0 = LA0;
					if (la0 == TT.Comma) {
						la1 = LA(1);
		...
		{
			// Line 241: ((TT.Not|TT.Id|TT.OtherLit|TT.Symbol|TT.NotBits|TT.At|TT.LParen|TT.Mul|TT.TypeKeyword|TT.Add|TT.LBrace|TT.Sub|TT.Forward|TT.IncDec|TT.String|TT.SQString|TT.AndBits|TT.Number|TT.@new|TT.Substitute) | UnusualId)
			 switch (LA0) {
			case TT.@new:
			case TT.Add:
			case TT.AndBits:
		...
		{
			TokenType la0;
			// Line 906: ( DataType ((TT.Id|TT.AttrKeyword|TT.TypeKeyword) | UnusualId) | (TT.AttrKeyword|TT.@new) | &{_spaceName != S.Def} @`.`(TT, noMacro(@this)) | TT.@checked TT.LBrace TT.RBrace | TT.@unchecked TT.LBrace TT.RBrace | @`.`(TT, noMacro(@default)) TT.Colon | TT.@using TT.LParen | (TT.@while|TT.@try|TT.@switch|TT.@interface|@`.`(TT, noMacro(@throw))|TT.@namespace|TT.@delegate|TT.@for|@`.`(TT, noMacro(@return))|TT.@do|TT.@event|TT.@foreach|TT.@struct|TT.@fixed|TT.@goto|TT.@lock|TT.@case|TT.@class|@`.`(TT, noMacro(@break))|TT.@enum|@`.`(TT, noMacro(@continue))) )
			 switch (LA0) {
			case TT.@operator:
			case TT.ContextualKeyword:
			case TT.Id:
			...
			case TT.TypeKeyword:
				{
					if (!Scan_DataType())
						return false;
					// Line 906: ((TT.Id|TT.AttrKeyword|TT.TypeKeyword) | UnusualId)
					la0 = LA0;
		...
		{
			TokenType la0;
			// Line 1426: ( TT.LParen TT.RParen (TT.LBrace TT.RBrace | TT.Id) | TT.LBrace TT.RBrace | TT.Forward )
			la0 = LA0;
			if (la0 == TT.LParen) {
				if (!TryMatch((int) TT.LParen))
					return false;
				if (!TryMatch((int) TT.RParen))
					return false;
				// Line 1426: (TT.LBrace TT.RBrace | TT.Id)
				la0 = LA0;
				if (la0 == TT.LBrace) {
					if (!TryMatch((int) TT.LBrace))
						return false;
