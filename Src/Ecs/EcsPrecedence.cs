...
namespace ecs
{
	...
	/// PF must drop to int.MinValue so that it is possible to parse <c>a?b=x:c</c> 
	/// even though '=' supposedly has lower precedence than the conditional 
	/// operator. Note that <c>a=b ? c=d : e=f</c> has the interpretation 
	/// <c>a=(b ? c=d : e)=f</c>, so you can see that the precedence of the 
	/// conditional operator is higher at the "edges".
	/// <para/>
	/// The above explanation illustrates the meaning of Left and Right. The actual
	/// EC# parser may or may not use the PF concept and PrecedenceRange objects.
	/// </remarks>
	public class Precedence
	{
		...
		public static bool operator ==(Precedence a, Precedence b) { return a.Lo == b.Lo && a.Hi == b.Hi; }
		public static bool operator !=(Precedence a, Precedence b) { return a.Lo != b.Lo || a.Hi != b.Hi; }
		...
		public override bool Equals(object obj) { return (obj is Precedence) && this == (obj as Precedence); }
		public override int  GetHashCode() { return Lo ^ (Hi << 4); }
		public static readonly Precedence Minimum = new Precedence(int.MinValue, int.MinValue, int.MinValue);
	}
	...
	/// <remarks>
	/// Summary:
	/// <br/>100+: Primary: x.y f(x) a[i] etc.
	...
	{
		public static readonly Precedence Substitute = new Precedence(102,103,102);    // \x     \x.y
		public static readonly Precedence Primary    = new Precedence(100,101,100);    // x.y x::y x:::y x->y f(x) x(->y) a[x] x++ x-- new typeof() checked() unchecked()
		...
		public static readonly Precedence Custom     = new Precedence(46,  73, 46,72); // `custom operator` (immiscible with * / + - << >>)
		...
		public static readonly Precedence Lambda     = new Precedence(-2,  -1, 95,-1); // =>
	}
}
