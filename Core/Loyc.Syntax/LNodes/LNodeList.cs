...
namespace Loyc.Syntax
{
	...
	{
		public static readonly LNodeList Empty = VList<LNode>.Empty;

		VList<LNode> _list;

		public static implicit operator LNodeList(VList<LNode> list) => new LNodeList { _list = list };
		...
		public LNodeList Add(LNode item) => _list.Add(item ?? throw new ArgumentNullException(nameof(item)));
		...
		public LNodeList Insert(int index, LNode item) => _list.Insert(index, item ?? throw new ArgumentNullException(nameof(item)));
		public bool Remove(LNode item) => _list.Remove(item);
		void IList<LNode>.RemoveAt(int index) => RemoveAt(index);
		public LNodeList RemoveAt(int index) => _list.RemoveAt(index);
		...
		/// </remarks>
		public LNodeList SmartSelect(Func<LNode, LNode> map) => _list.SmartSelect(map);
		...
		/// that often processes a list without modifying it at all.
		/// </remarks>
		public LNodeList SmartSelectMany(Func<LNode, IList<LNode>> map) => _list.SmartSelectMany(map);

		/// <summary>Filters the list, returning the same list if the filter function returns true for every item.</summary>
		public LNodeList SmartWhere(Func<LNode, bool> filter) => _list.SmartWhere(filter);
		...
		/// new one.
		/// </remarks>
		public LNodeList WhereSelect(Func<LNode, Maybe<LNode>> filter) => _list.WhereSelect(filter);

		public LNodeList AddRange(VList<LNode> list) => _list.AddRange(list);
		public LNodeList AddRange(LNodeList list) => _list.AddRange(list._list);
		public LNodeList AddRange(IList<LNode> list) => _list.AddRange(list);
		public LNodeList AddRange(IEnumerable<LNode> list) => _list.AddRange(list);
		public LNodeList InsertRange(int index, IList<LNode> list) => _list.InsertRange(index, list);
		public LNodeList RemoveRange(int index, int count) => _list.RemoveRange(index, count);
