...
namespace Loyc.Runtime
{
	...
	/// for developers.
	/// </summary><remarks>
	/// All Loyc code should call this hook in order to localize text. Use it
	/// like this:
	/// <code>
	/// string result = Localize.From("Hello, {0}", userName);
	/// </code>.
	/// If you use this facility frequently in a given class, you may want to 
	/// shorten your typing using a static variable:
	/// <code>
	...
	/// whatever localizer is installed will look up the text in its database
	/// and return a translation. If no translation to the end user's language is 
	/// available, an appropriate default translation should be returned: either
	/// the original text, or a translation to some default language, e.g. English.
	/// 
	/// Alternately, assuming you have the ability to change the table of 
	/// translations, you can use a Symbol in your code and call the other 
	/// overload of From() to look up the text that should be shown to the end 
	/// user:
	/// <code>
	/// string result = Localize.From(Symbol.Get("MY_STRING"));
	/// string result = Localize.From(:MY_STRING); // Loyc syntax
	/// </code>
	/// This is most useful for long strings or paragraphs of text, but I 
	/// expect that many projects, as a policy, will use symbols for all 
	/// localizable text.
	///
	/// Localize.Formatter() is then called to make the completed string, unless the 
	/// variable argument list is empty. Therefore, it is also possible to perform 
	/// formatting manually:
	/// <code>
	/// Console.WriteLine(Localize.From("HEXER\0{0} is {0:X} in hexadecimal"), N);
	/// </code>
	...
	/// (1) It may be cumbersome to pass around a ResourceManager instance between 
	///     all classes that might contain localizable strings; a global facility 
	///     is much more convenient.
	/// (2) The programmer has to put all translations in the resource file; 
	///     consequently, writing the code is bothersome because the programmer
	///     has to switch to the resource file and add the string to it. Someone 
	///     reading the code, in turn, can't tell what the string says and has to
	///     load up the resource file to find out.
	/// (3) It is nontrivial to change the localization manager; for instance, what
	///     if someone wants to store translations in an .ini or .xml file rather
	...
	/// (4) Keeping in mind that the guy in charge of translation is typically
	///     different than the guys writing most of the code, it makes sense to 
	///     keep translations separate from everything else.
	/// 
	/// The idea of the Localize facility is to convince programmers to support
	/// localization by making it dead-easy to do. By default it is not 
	/// connected to any translator (it just passes strings through), so people
	/// who are only writing a program for a one-language market can easily 
	/// make their code "multiligual-ready" without doing any extra work, since 
	/// Localize.From() is no harder to type than String.Format().
	/// 
	/// The translation system itself is separate, and connected to Localize by 
	/// a delegate, for two reasons:
	/// (1) Multiple translation systems are possible. This class should be 
	///     suitable for any .NET program, and some programs using this utility 
	///     will want to plug-in a different localizer.
	/// (2) I personally don't have the time or expertise to write a localizer
	///     at this time. So until I do, the Localize class will make my code
	///     ready for translation, although not actually localized.
	/// 
	...
	/// many different individuals--not teams--of programmers will create and 
	/// maintain features. By centralizing this translation facility, it should 
	...
	/// time) for calls to methods in this class and generate a list of strings 
	/// and symbols needing translation. It would also have to detect certain 
	/// calls that perform translation implicity, such as ISimpleMessageSink.
	/// Write().
	/// </remarks>
	public static class Localize
	{
		...
		/// <summary>
		/// This is the heart of the Localize class, which l
		/// </summary>
		/// <param name="msgId">Resource ID used to look up a string. If msgId.IsNull 
		/// then sourceLang and msg must be provided; otherwise, sourceLang and msg 
		/// are only used if no translation is available for the specified ID. The 
		/// translator may also use msg if sourceLang matches the active destination 
		/// language.</param>
		/// <param name="sourceLang">Optional (can be null) language code of the language 
		/// that s is written in (e.g. "EN" for English).</param>
		/// <param name="msg">The message to translate (typically ignored if 
		/// !msgId.IsNull), which may include argument placeholders (e.g. "{0}"). As 
		/// explained in the remarks for this class, msg may also include a resource 
		/// ID.</param>
		/// <param name="args">Arguments given to Formatter to fill in placeholders after 
		/// the Localizer is called. If args is null or empty then Formatter is not 
		/// called.</param>
		/// <returns>The translated and formatted string.</returns>
		/// <remarks>
		/// Do() looks for a resource ID embedded in msg only if msgId.IsNull. If an 
		/// embedded ID is present, it is stripped out, converted to a symbol, and becomes
		/// msgId. Thus, Localizer is never given a message with an embedded ID.
		/// </remarks>
