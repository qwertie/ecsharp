...
namespace Loyc.Runtime
{
	...
	/// <para/>
	/// For example, JPTrie may require a temporary byte array during searches.
	/// Re-creating the byte array for every search might cause a too much time
	/// to be spent garbage-collecting. On the other hand, if JPTrie keeps a 
	/// reference to the temporary buffer in itself, what if a program contains 
	/// many instances of JPTrie? Each one would have its own separate temporary
